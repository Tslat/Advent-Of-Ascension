...
public class EntityHolderGoldorth extends EntityAnimal implements CavernBoss {
	private int countDown;

	public EntityHolderGoldorth(final World par1World) {
		super(par1World);
		countDown = 700;
		setSize(0.1f, 0.1f);
	...
	public EntityAgeable createChild(final EntityAgeable var1) {
		return new EntityHolderGoldorth(worldObj);
	}
	...
	public void onUpdate() {
		...
		if (countDown == 0) {
			if (!worldObj.isRemote) {
				final EntityGoldorth var2 = new EntityGoldorth(worldObj);
				var2.setLocationAndAngles(posX, posY + 2, posZ, rand.nextFloat() * 360.0f, 0.0f);
				worldObj.spawnEntityInWorld(var2);
				playSound("nevermine:GoldorthSpawn", 0.85f, 1.0f);
			}
			setDead();
		}
	}
