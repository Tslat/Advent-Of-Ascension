...
public class EntityHolderConiferon extends EntityAnimal implements CavernBoss {
	private int countDown;

	public EntityHolderConiferon(final World par1World) {
		super(par1World);
		countDown = 700;
		setSize(0.1f, 0.1f);
	...
	public EntityAgeable createChild(final EntityAgeable var1) {
		return new EntityHolderConiferon(worldObj);
	}
	...
	public void onUpdate() {
		...
		if (countDown == 0) {
			if (!worldObj.isRemote) {
				final EntityConiferon var2 = new EntityConiferon(worldObj);
				var2.setLocationAndAngles(posX, posY + 2, posZ, rand.nextFloat() * 360.0f, 0.0f);
				worldObj.spawnEntityInWorld(var2);
				playSound("nevermine:ConiferonSpawn", 0.85f, 1.0f);
			}
			setDead();
		}
	}
