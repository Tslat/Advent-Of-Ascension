...
public class EntityHolderHoron extends EntityAnimal implements CavernBoss {
	private int countDown;

	public EntityHolderHoron(final World par1World) {
		super(par1World);
		countDown = 700;
		setSize(0.1f, 0.1f);
	...
	public EntityAgeable createChild(final EntityAgeable var1) {
		return new EntityHolderHoron(worldObj);
	}
	...
	public void onUpdate() {
		...
		if (countDown == 0) {
			if (!worldObj.isRemote) {
				final EntityHoron var2 = new EntityHoron(worldObj);
				var2.setLocationAndAngles(posX, posY + 2, posZ, rand.nextFloat() * 360.0f, 0.0f);
				worldObj.spawnEntityInWorld(var2);
				playSound("nevermine:HoronSpawn", 0.85f, 1.0f);
			}
			setDead();
		}
	}
