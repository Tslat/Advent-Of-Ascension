...
public class GrassDustopia extends Block implements IGrowable {
	...
	public GrassDustopia(final Material p_i45394_1_) {
		super(p_i45394_1_);
		setCreativeTab(Itemizer.GenerationTab);
		setHardness(1.5f);
		setResistance(0.2f);
		setStepSound(GrassDustopia.grass);
	...
	public Item getItemDropped(final int par1, final Random par2, final int par3) {
		return Item.getItemFromBlock(Blockizer.DirtDustopia);
	}

	public void updateTick(final World world, final int x, final int y, final int z, final Random random) {
		if (!world.isRemote) {
			if (world.getBlockLightValue(x, y + 1, z) < 4 && world.getBlockLightOpacity(x, y + 1, z) > 2) {
				world.setBlock(x, y, z, Blockizer.DirtDustopia);
			}
				...
				for (int l = 0; l < 4; ++l) {
					...
					if (world.getBlock(i1, j1, k1) == Blockizer.DirtDustopia && world.getBlockLightValue(i1, j1 + 1, k1) >= 4 && world.getBlockLightOpacity(i1, j1 + 1, k1) <= 2) {
	...
	public void registerBlockIcons(final IIconRegister icon) {
		top = icon.registerIcon("nevermine:grassDustopia_top");
		side = icon.registerIcon("nevermine:grassDustopia_side");
		bottom = icon.registerIcon("nevermine:dirtDustopia");
		blockIcon = icon.registerIcon("nevermine:grassDustopia_side");
	}

	static {
		GrassDustopia.grass = Block.soundTypeGrass;
	}
}
