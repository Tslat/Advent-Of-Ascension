...
public class GrassRunic extends Block implements IGrowable {
	private static Block.SoundType rock;
	...
	public GrassRunic(final Material p_i45394_1_) {
		super(p_i45394_1_);
		setCreativeTab(Itemizer.GenerationTab);
		setHardness(1.5f);
		setResistance(0.2f);
		setStepSound(GrassRunic.rock);
	...
	public Item getItemDropped(final int par1, final Random par2, final int par3) {
		return Item.getItemFromBlock(Blockizer.RunicBrick);
	}

	public void updateTick(final World world, final int x, final int y, final int z, final Random random) {
		if (!world.isRemote) {
			if (world.getBlockLightValue(x, y + 1, z) < 4 && world.getBlockLightOpacity(x, y + 1, z) > 2) {
				world.setBlock(x, y, z, Blockizer.RunicBrick);
			}
				...
				for (int l = 0; l < 4; ++l) {
					...
					if (world.getBlock(i1, j1, k1) == Blockizer.RunicBrick && world.getBlockLightValue(i1, j1 + 1, k1) >= 4 && world.getBlockLightOpacity(i1, j1 + 1, k1) <= 2) {
	...
	public void registerBlockIcons(final IIconRegister icon) {
		top = icon.registerIcon("nevermine:grassRunic_top");
		side = icon.registerIcon("nevermine:grassRunic_side");
		bottom = icon.registerIcon("nevermine:runicBrick");
		blockIcon = icon.registerIcon("nevermine:grassRunic_side");
	}

	static {
		GrassRunic.rock = Block.soundTypeStone;
	}
}
