...
public class GoldAccumulator extends Block {
	...
	public boolean onBlockActivated(final World w, final int x, final int y, final int z, final EntityPlayer p, final int var6, final float var7, final float var8, final float var9) {
		if (!w.isRemote && !p.isSneaking() && p.inventory.consumeInventoryItem(Itemizer.ImpureGold)) {
			...
			if (cont.getTribute(Pluton) == 100)
				p.addChatMessage(StringUtil.getLocaleWithArguments("message.feedback.immortallisProgression.goldEnd", p.getDisplayName()));
		}
		return true;
	}
