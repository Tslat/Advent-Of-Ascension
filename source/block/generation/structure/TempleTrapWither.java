...
public class TempleTrapWither extends Block {
	public TempleTrapWither(final Material p_i45394_1_) {
		super(p_i45394_1_);
	...
	public void onEntityCollidedWithBlock(final World w, final int x, final int y, final int z, final Entity e) {
		if (w.getBlock(x, y, z) == Blockizer.TempleTrapWither && e instanceof EntityPlayer && !((EntityPlayer)e).capabilities.isCreativeMode) {
			((EntityLivingBase)e).addPotionEffect(new PotionEffect(Potion.wither.id, 40, 3));
			e.worldObj.playSoundAtEntity(e, "nevermine:TempleLaugh", 1.3f, 1.0f);
			((EntityLivingBase)e).addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 40, 3));
		}
	}
}
