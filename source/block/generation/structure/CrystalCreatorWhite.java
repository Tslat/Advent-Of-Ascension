...
public class CrystalCreatorWhite extends Block {
	private Random rand;

	public CrystalCreatorWhite(final Material p_i45394_1_) {
		super(p_i45394_1_);
		rand = new Random();
	...
	public boolean onBlockActivated(final World w, final int x, final int y, final int z, final EntityPlayer p, final int var6, final float var7, final float var8, final float var9) {
		if (!w.isRemote && p.getHeldItem() != null && p.getHeldItem().getItem() == Itemizer.GemstonesWhite) {
			p.inventory.consumeInventoryItem(Itemizer.GemstonesWhite);
			if (!w.isRemote && rand.nextInt(8) == 2) {
				p.dropItem(Itemizer.CrystalsWhite, 1);
