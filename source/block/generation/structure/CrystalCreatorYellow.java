...
public class CrystalCreatorYellow extends Block {
	private Random rand;

	public CrystalCreatorYellow(final Material p_i45394_1_) {
		super(p_i45394_1_);
		rand = new Random();
	...
	public boolean onBlockActivated(final World w, final int x, final int y, final int z, final EntityPlayer p, final int var6, final float var7, final float var8, final float var9) {
		if (!w.isRemote && p.getHeldItem() != null && p.getHeldItem().getItem() == Itemizer.GemstonesYellow) {
			p.inventory.consumeInventoryItem(Itemizer.GemstonesYellow);
			if (!w.isRemote && rand.nextInt(8) == 2) {
				p.dropItem(Itemizer.CrystalsYellow, 1);
