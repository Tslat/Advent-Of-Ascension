...
public class BlockAcid extends Block {
	...
	private boolean canSnowStay(final World world, final int par2, final int par3, final int par4) {
		if (!world.isRemote && !canPlaceBlockAt(world, par2, par3, par4)) {
	...
	public void updateTick(final World world, final int par2, final int par3, final int par4, final Random par5Random) {
		if (!world.isRemote)
			world.setBlockToAir(par2, par3, par4);
	}

	public void onEntityCollidedWithBlock(final World var1, final int var2, final int var3, final int var4, final Entity var5) {
		if (var5 instanceof EntityPlayer && !((EntityPlayer)var5).capabilities.isCreativeMode) {
			var5.attackEntityFrom(new DamageSource("slugAcid"), 15.0f);
			((EntityLivingBase)var5).addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 40, 3));
		}
	}
