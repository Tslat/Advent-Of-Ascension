...
public class HeavyShadowBallRenderer extends Render<EntityHeavyShadowball> {
	private final ResourceLocation texture;
	private int counter = 12;
	private boolean toggle;

	public HeavyShadowBallRenderer(final RenderManager manager, final ResourceLocation textureResource) {
		super(manager);
		texture = textureResource;
	}

	@Override
	public void doRender(EntityHeavyShadowball entity, double x, double y, double z, float entityYaw, float partialTicks) {
		...
		for (int i = 0; i < 8; i++) {
			...
			if (toggle) {
				entity.world.spawnParticle(ParticleRegister.FLICKERING_FLUFFY, entity.posX, entity.posY, entity.posZ, 0, 0, 0, Enums.RGBIntegers.PURPLE, 100, 3, 7);
			}
			else {
	...
	@Nullable
	@Override
	protected ResourceLocation getEntityTexture(EntityHeavyShadowball entity) {
		return texture;
	}
}
