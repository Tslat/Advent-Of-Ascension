...
public class LightBlasterShotRenderer extends ParticleProjectileRenderer<LightBlasterShotEntity> {
	...
	protected void addParticles(LightBlasterShotEntity entity, float partialTicks) {
		for (int i = 0; i < 3; i++) {
			...
			for (int j = 0; j < 3; j++) {
				entity.world.addParticle(new CustomisableParticleType.Data(AoAParticleTypes.FLICKERING_SPARKLER.get(), 1, 3, NumberUtil.RGB(0, 255, 0)), entity.getPosX(), entity.getPosY() + entity.yOffset1, entity.getPosZ(), 0, 0, 0);
