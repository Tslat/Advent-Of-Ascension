...
import net.tslat.aoa3.object.entity.projectile.gun.SunsetBulletEntity;
...
public class SunsetBulletRenderer extends TexturedProjectileRenderer<SunsetBulletEntity> {
	...
	public void render(SunsetBulletEntity entity, float entityYaw, float partialTicks, MatrixStack matrix, IRenderTypeBuffer buffer, int packedLight) {
		...
		for (int i = 0; i < 8; i++) {
			entity.level.addParticle(new CustomisableParticleType.Data(AoAParticleTypes.SPARKLER.get(), 1, 3, ColourUtil.RGB(223, 153, 0)), entity.getX(), entity.getY() + 0.5, entity.getZ(), 0, 0, 0);
			entity.level.addParticle(new CustomisableParticleType.Data(AoAParticleTypes.SPARKLER.get(), 1, 3, ColourUtil.BLACK), entity.getX(), entity.getY() - 0.5, entity.getZ(), 0, 0, 0);
		}
	}
}