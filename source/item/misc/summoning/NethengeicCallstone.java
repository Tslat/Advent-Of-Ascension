...
public class NethengeicCallstone extends BossSpawningItem {
	...
	public void spawnBoss(World world, ServerPlayerEntity summoner, double posX, double posY, double posZ) {
		...
		nethengeicWither.setLocationAndAngles(posX, posY, posZ, RandomUtil.randomValueUpTo(360f), 0f);
		world.addEntity(nethengeicWither);
		PlayerUtil.messageAllPlayersInRange(LocaleUtil.getLocaleMessage("entity.aoa3.nethengeic_wither.spawn", summoner.getDisplayName().getFormattedText()), world, new BlockPos(posX, posY, posZ), 50);
	}
	...
	public boolean canSpawnHere(World world, ServerPlayerEntity player, double posX, double posY, double posZ) {
		...
		if (world.checkBlockCollision(new AxisAlignedBB(posX - 1d, posY, posZ - 1d, posX + 1d, posY + 2.5d, posZ + 1d))) {
