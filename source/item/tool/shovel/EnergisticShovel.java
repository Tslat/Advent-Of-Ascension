...
public class EnergisticShovel extends ItemSpade implements SkillItem {
	public EnergisticShovel(ToolMaterial material) {
		super(material);
		setUnlocalizedName("EnergisticShovel");
		setRegistryName("aoa3:energistic_shovel");
	...
	public boolean onBlockDestroyed(ItemStack stack, World world, IBlockState blockState, BlockPos pos, EntityLivingBase entity) {
		...
		if (!world.isRemote && ToolRegister.shovelEnergistic.getDestroySpeed(stack, blockState) > 1 && blockState.getBlockHardness(world, pos) > 0) {
			if (itemRand.nextInt(5) == 0)
				blockState.getBlock().dropXpOnBlockBreak(world, pos, itemRand.nextInt(3) + 1);
		}
	...
	public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag flag) {
		tooltip.add(StringUtil.getColourLocaleString("item.EnergisticShovel.desc.1", TextFormatting.DARK_GREEN));
