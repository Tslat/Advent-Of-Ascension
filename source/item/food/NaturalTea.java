...
public class NaturalTea extends ItemFood {
	public NaturalTea() {
		super(0, 0.5f, true);
	...
	public ItemStack onEaten(final ItemStack item, final World world, final EntityPlayer player) {
		...
		if (!world.isRemote) {
			player.addPotionEffect(new PotionEffect(Potion.regeneration.id, 50));
			player.addPotionEffect(new PotionEffect(Potion.resistance.id, 250, 1));
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		list.add(StringUtil.getLocaleString("item.NaturalTea.desc.1"));
	}
}
