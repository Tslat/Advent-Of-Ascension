...
public class UntiringTablet extends TabletItem {
	public UntiringTablet(String name, String registryName, float placementCost, float tickSoulDrain, int levelReq, int effectRadius) {
	...
	protected EntitySoulTablet getTabletEntity(World world, EntityPlayer placer) {
		return new EntityUntiringTablet(world, placer);
	}

	@SideOnly(Side.CLIENT)
	@Override
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
		tooltip.add(StringUtil.getLocaleString("item.UntiringTablet.desc.1"));
