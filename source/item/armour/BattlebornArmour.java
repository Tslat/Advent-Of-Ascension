...
public class BattlebornArmour extends AdventArmour {
	...
	public BattlebornArmour(EquipmentSlotType slot) {
		super(ItemUtil.customArmourMaterial("aoa3:battleborn", 65, new int[] {4, 8, 9, 5}, 10, SoundEvents.ITEM_ARMOR_EQUIP_GENERIC, 7), slot);
	}
	...
	public void onDamageDealt(PlayerDataManager plData, @Nullable HashSet<EquipmentSlotType> slots, LivingHurtEvent event) {
		if (slots != null && DamageUtil.isMeleeDamage(event.getSource())) {
			...
			EntityUtil.removeAttributeModifier(plData.player(), SharedMonsterAttributes.ATTACK_SPEED, BATTLEBORN_ARMOUR_BUFF);
			EntityUtil.applyAttributeModifierSafely(plData.player(), SharedMonsterAttributes.ATTACK_SPEED, buff(Math.min(0.65, newAmount / 240d)));
		}
	}
	...
	public void onEffectTick(PlayerDataManager plData, @Nullable HashSet<EquipmentSlotType> slots) {
		...
		else if (counter > 0 && plData.player().world.getGameTime() % 10 == 0) {
			EntityUtil.removeAttributeModifier(plData.player(), SharedMonsterAttributes.ATTACK_SPEED, BATTLEBORN_ARMOUR_BUFF);
			EntityUtil.applyAttributeModifierSafely(plData.player(), SharedMonsterAttributes.ATTACK_SPEED, buff(Math.min(0.65, counter / 240d)));
		}
	}
