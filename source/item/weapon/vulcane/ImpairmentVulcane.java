...
public class ImpairmentVulcane extends BaseVulcane {
	...
	public ImpairmentVulcane(final String effect, final int uses, final float dmg, final int need) {
	...
	public void fireGun(final World world, final ItemStack stack, final EntityPlayer player, final float multi, final EntityMob mob) {
		mob.addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 80, 2));
	}
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		...
		list.add(StringUtil.getColourLocaleString("item.ImpairmentVulcane.desc.1", EnumChatFormatting.DARK_GREEN));
		list.add(StringUtil.getColourLocaleString("items.description.vulcane.activate", EnumChatFormatting.AQUA));
		list.add(StringUtil.getColourLocaleStringWithArguments("items.description.skillRequirement", EnumChatFormatting.RED, Integer.toString(20), StringUtil.getLocaleString("skills.vulcanism.name")));
	}
}
