...
public class SubterraneanGreatblade extends BaseGreatblade implements AdventWeapon, LongReachWeapon {
	public SubterraneanGreatblade(double dmg, double speed, int durability) {
		...
		setUnlocalizedName("SubterraneanGreatblade");
	...
	public void attackEntity(ItemStack stack, Entity target, EntityLivingBase attacker, float dmg) {
		...
		if (target instanceof EntityLivingBase && ItemUtil.checkCooledItemProc(attacker, 0.33f))
			((EntityLivingBase)target).addPotionEffect(new PotionEffect(MobEffects.SLOWNESS, 30, 50));
	}

	@SideOnly(Side.CLIENT)
	public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
		tooltip.add(StringUtil.getColourLocaleString("item.SubterraneanGreatblade.desc.1", TextFormatting.DARK_GREEN));
	}
}
