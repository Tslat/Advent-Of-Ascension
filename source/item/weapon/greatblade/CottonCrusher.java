...
public class CottonCrusher extends BaseGreatblade {
	...
	public CottonCrusher(final int uses, final float dealDamage) {
	...
	public void fireGreatblade(final World world, final ItemStack stack, final EntityPlayer player) {
		player.worldObj.spawnEntityInWorld(new EntityGreatblade(player.worldObj, player, dmg, 13));
	}
	...
	public void addInformation(final ItemStack item, final EntityPlayer player, final List list, final boolean bool) {
		...
		list.add(StringUtil.getColourLocaleString("item.CottonCrusher.desc.1", EnumChatFormatting.DARK_GREEN));
