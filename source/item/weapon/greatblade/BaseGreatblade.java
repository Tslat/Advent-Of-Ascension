...
public abstract class BaseGreatblade extends Item {
	...
	public BaseGreatblade(final int uses, final float dealDamage) {
		rand = new Random();
		canShootTick = 0;
		setCreativeTab(Weaponizer.GreatbladesTab);
		dmg = dealDamage;
		setMaxDamage(uses);
		setMaxStackSize(1);
		setFull3D();
	...
	public boolean onEntitySwing(final EntityLivingBase player, final ItemStack stack) {
		...
		if (Ticker.tick >= stack.getTagCompound().getLong("lastShotTime") && player instanceof EntityPlayer && !player.worldObj.isRemote) {
			fireGreatblade(player.worldObj, stack, (EntityPlayer)player);
	...
	public abstract void fireGreatblade(final World p0, final ItemStack p1, final EntityPlayer p2);
}
