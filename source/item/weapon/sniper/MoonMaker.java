...
public class MoonMaker extends BaseGun implements SniperInstance {
	private final float dmg = 75.0f;
	private float multiplier;

	public MoonMaker(final int consumeChance, final String effect, final int uses, final int fireRate, final Item item) {
	...
	public void fireGun(final World world, final ItemStack stack, final EntityPlayer player, final float multi, boolean consume) {
		...
		player.worldObj.spawnEntityInWorld(new EntityMoonMaker(player.worldObj, player, dmg * multiplier * multi, 0));
	}

	@SideOnly(Side.CLIENT)
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		list.add(StringUtil.getColourLocaleStringWithArguments("items.description.damage.ranged", EnumChatFormatting.DARK_RED, Integer.toString((int)dmg)));
		list.add(StringUtil.getColourLocaleString("item.MoonMaker.desc.1", EnumChatFormatting.DARK_GREEN));
