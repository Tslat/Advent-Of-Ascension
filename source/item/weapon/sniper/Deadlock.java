...
public class Deadlock extends BaseGun implements SniperInstance {
	private final float dmg = 40.0f;
	private float multiplier;

	public Deadlock(final int consumeChance, final String effect, final int uses, final int fireRate, final Item item) {
	...
	public void fireGun(final World world, final ItemStack stack, final EntityPlayer player, final float multi, boolean consume) {
		...
		player.worldObj.spawnEntityInWorld(new EntityPenetrationBullet(player.worldObj, player, 40.0f * multiplier * multi, 4));
	}

	@SideOnly(Side.CLIENT)
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		list.add(StringUtil.getColourLocaleStringWithArguments("items.description.damage.ranged", EnumChatFormatting.DARK_RED, Integer.toString((int)dmg)));
		list.add(StringUtil.getColourLocaleString("item.Deadlock.desc.1", EnumChatFormatting.DARK_GREEN));
