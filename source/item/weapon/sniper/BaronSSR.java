...
public class BaronSSR extends BaseGun implements SniperInstance {
	private final float dmg = 40.0f;
	private float multiplier;

	public BaronSSR(final int consumeChance, final String effect, final int uses, final int fireRate, final Item item) {
	...
	public void fireGun(final World world, final ItemStack stack, final EntityPlayer player, final float multi, boolean consume) {
		multiplier = ArmorUtil.isSharpshotArmor(player) ? 1.25f : 1.0f;
