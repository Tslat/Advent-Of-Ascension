...
public class Floro500 extends BaseGun implements SniperInstance {
	private final float dmg = 70.0f;
	private float multiplier;

	public Floro500(final int consumeChance, final String effect, final int uses, final int fireRate, final Item item) {
	...
	public void fireGun(final World world, final ItemStack stack, final EntityPlayer player, final float multi, boolean consume) {
		...
		player.worldObj.spawnEntityInWorld(new EntityFloroMetalPellet(player.worldObj, player, dmg * multiplier * multi, 0));
	}
