		...
		if (target instanceof LivingEntity && target.level instanceof ServerWorld) {
			...
			if (preciseImpactSpot != null) {
				...
				if (preciseImpactSpot.y > headMinRange && preciseImpactSpot.y < headMaxRange) {
					for (int i = 0; i < 5; i++) {
						((ServerWorld)target.level).sendParticles(ParticleTypes.DAMAGE_INDICATOR, preciseImpactSpot.x + random.nextDouble() - 0.5d, preciseImpactSpot.y + random.nextDouble() - 0.5d, preciseImpactSpot.z + random.nextDouble() - 0.5d, 3, 0, 0, 0, 0);
					}
