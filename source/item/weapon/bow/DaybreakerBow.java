...
public class DaybreakerBow extends ItemBow {
	...
	public DaybreakerBow(final int uses, final int damage, final Item arrow) {
		this(uses, damage, 72000, arrow);
		setFull3D();
	}

	public DaybreakerBow(final int uses, final int damage, final int maxUseDuraction, final Item arrow) {
		setMaxDamage(uses);
		this.arrow = arrow;
		this.damage = damage;
	...
	public void registerIcons(final IIconRegister icon) {
		final String prefix = "nevermine:animateditem/bowDaybreaker";
		itemIcon = icon.registerIcon(prefix + "_0");
		IIconArray = new IIcon[DaybreakerBow.texture.length];
		for (int N = 0; N < 4; ++N) {
			IIconArray[N] = icon.registerIcon(prefix + DaybreakerBow.texture[N]);
		}
	}
	...
	public void onPlayerStoppedUsing(final ItemStack item, final World par2World, final EntityPlayer par3EntityPlayer, final int par4) {
		...
		if (flag || par3EntityPlayer.inventory.hasItem(arrow)) {
			...
			EntityArrow entityarrow3 = null;
			entityarrow = new EntityElementalArrow(par2World, par3EntityPlayer, f * 1.5f, 0, false);
			entityarrow2 = new EntityElementalArrow(par2World, par3EntityPlayer, f * 0.7f, 0, false);
			entityarrow3 = new EntityElementalArrow(par2World, par3EntityPlayer, f * 1.1f, 0, false);
			if (f == 1.0f) {
			...
			if (k > 0) {
				...
				entityarrow2.setDamage(entityarrow.getDamage() + k * 0.5 + 0.5);
				entityarrow3.setDamage(entityarrow.getDamage() + k * 0.5 + 0.5);
			}
			...
			if (sound(item, par2World, par3EntityPlayer) != null) {
				par2World.playSoundAtEntity(par3EntityPlayer, sound(item, par2World, par3EntityPlayer), 1.0f, 1.0f / (DaybreakerBow.itemRand.nextFloat() * 0.4f + 1.2f) + f * 0.5f);
			}
			if (flag) {
	...
	public void addInformation(final ItemStack item, final EntityPlayer player, final List list, final boolean bool) {
		...
		list.add(StringUtil.getColourLocaleString("item.DaybreakerBow.desc.1", EnumChatFormatting.DARK_GREEN));
		list.add(StringUtil.getColourLocaleString("item.DaybreakerBow.desc.2", EnumChatFormatting.DARK_GREEN));
