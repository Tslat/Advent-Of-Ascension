...
public class HauntedBow extends ItemBow {
	...
	public HauntedBow(final int uses, final int damage, final Item arrow) {
		this(uses, damage, 72000, arrow);
		setFull3D();
	}

	public HauntedBow(final int uses, final int damage, final int maxUseDuraction, final Item arrow) {
		setMaxDamage(uses);
		this.arrow = arrow;
		this.damage = damage;
	...
	public void registerIcons(final IIconRegister icon) {
		final String prefix = "nevermine:animateditem/bowHaunted";
		itemIcon = icon.registerIcon(prefix + "_0");
		IIconArray = new IIcon[HauntedBow.texture.length];
		for (int N = 0; N < 4; ++N) {
			IIconArray[N] = icon.registerIcon(prefix + HauntedBow.texture[N]);
		}
	}
	...
	public void onPlayerStoppedUsing(final ItemStack item, final World par2World, final EntityPlayer par3EntityPlayer, final int par4) {
		...
		if (flag || par3EntityPlayer.inventory.hasItem(arrow)) {
			...
			if (!par3EntityPlayer.capabilities.isCreativeMode) {
				if (par3EntityPlayer.getHealth() > 1.0f) {
					par3EntityPlayer.setHealth(par3EntityPlayer.getHealth() - 1.0f);
				}
				else {
					par3EntityPlayer.attackEntityFrom(DamageSource.magic, 1.0f);
				}
			}

			entityarrow = new EntityElementalArrow(par2World, par3EntityPlayer, f * 2.0f, 12, false);
			if (f == 1.0f) {
			...
			if (sound(item, par2World, par3EntityPlayer) != null) {
				par2World.playSoundAtEntity(par3EntityPlayer, sound(item, par2World, par3EntityPlayer), 1.0f, 1.0f / (HauntedBow.itemRand.nextFloat() * 0.4f + 1.2f) + f * 0.5f);
			}
			if (flag) {
	...
	public void addInformation(final ItemStack item, final EntityPlayer player, final List list, final boolean bool) {
		list.add(StringUtil.getColourLocaleStringWithArguments("items.description.damage.rangedMax", EnumChatFormatting.DARK_RED, Integer.toString((int)damage + 8)));
		list.add(StringUtil.getColourLocaleString("item.HauntedBow.desc.1", EnumChatFormatting.DARK_GREEN));
