...
public class PrimordialBow extends ItemBow {
	...
	public PrimordialBow(final int uses, final int damage, final Item arrow) {
		this(uses, damage, 72000, arrow);
		setFull3D();
	}

	public PrimordialBow(final int uses, final int damage, final int maxUseDuraction, final Item arrow) {
		setMaxDamage(uses);
		this.arrow = arrow;
		this.damage = damage;
	...
	public void registerIcons(final IIconRegister icon) {
		final String prefix = "nevermine:animateditem/bowPrimordial";
		itemIcon = icon.registerIcon(prefix + "_0");
		IIconArray = new IIcon[PrimordialBow.texture.length];
		for (int N = 0; N < 4; ++N) {
			IIconArray[N] = icon.registerIcon(prefix + PrimordialBow.texture[N]);
		}
	}
	...
	public void onPlayerStoppedUsing(final ItemStack item, final World par2World, final EntityPlayer par3EntityPlayer, final int par4) {
		...
		if (flag || par3EntityPlayer.inventory.hasItem(arrow)) {
			...
			if (!par3EntityPlayer.capabilities.isCreativeMode) {
				if (par3EntityPlayer.getHealth() > 3.0f) {
					par3EntityPlayer.setHealth(par3EntityPlayer.getHealth() - 3.0f);
				}
				else {
					par3EntityPlayer.attackEntityFrom(DamageSource.magic, 3.0f);
				}
			}

			entityarrow = new EntityElementalArrow(par2World, par3EntityPlayer, f * 2.0f, 14, false);
			if (f == 1.0f) {
			...
			if (sound(item, par2World, par3EntityPlayer) != null) {
				par2World.playSoundAtEntity(par3EntityPlayer, sound(item, par2World, par3EntityPlayer), 1.0f, 1.0f / (PrimordialBow.itemRand.nextFloat() * 0.4f + 1.2f) + f * 0.5f);
			}
			if (flag) {
	...
	public void addInformation(final ItemStack item, final EntityPlayer player, final List list, final boolean bool) {
		...
		list.add(StringUtil.getColourLocaleString("item.PrimordialBow.desc.1", EnumChatFormatting.DARK_GREEN));
		list.add(StringUtil.getColourLocaleString("item.PrimordialBow.desc.2", EnumChatFormatting.DARK_GREEN));
