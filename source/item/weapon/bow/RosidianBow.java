...
public class RosidianBow extends ItemBow {
	...
	public RosidianBow(final int uses, final int damage, final Item arrow) {
		this(uses, damage, 72000, arrow);
		setFull3D();
	}

	public RosidianBow(final int uses, final int damage, final int maxUseDuraction, final Item arrow) {
		setMaxDamage(uses);
		this.arrow = arrow;
		this.damage = damage;
	...
	public void registerIcons(final IIconRegister icon) {
		final String prefix = "nevermine:animateditem/bowRosidian";
		itemIcon = icon.registerIcon(prefix + "_0");
		IIconArray = new IIcon[RosidianBow.texture.length];
		for (int N = 0; N < 4; ++N) {
			IIconArray[N] = icon.registerIcon(prefix + RosidianBow.texture[N]);
		}
	}
	...
	public void onPlayerStoppedUsing(final ItemStack item, final World par2World, final EntityPlayer par3EntityPlayer, final int par4) {
		...
		if (flag || par3EntityPlayer.inventory.hasItem(arrow)) {
			...
			entityarrow = new EntityElementalArrow(par2World, par3EntityPlayer, f * 2.0f, 16, false);
			if (f == 1.0f) {
			...
			if (sound(item, par2World, par3EntityPlayer) != null) {
				par2World.playSoundAtEntity(par3EntityPlayer, sound(item, par2World, par3EntityPlayer), 1.0f, 1.0f / (RosidianBow.itemRand.nextFloat() * 0.4f + 1.2f) + f * 0.5f);
			}
			if (flag) {
	...
	public void addInformation(final ItemStack item, final EntityPlayer player, final List list, final boolean bool) {
		...
		list.add(StringUtil.getColourLocaleString("item.RosidianBow.desc.1", EnumChatFormatting.DARK_GREEN));
