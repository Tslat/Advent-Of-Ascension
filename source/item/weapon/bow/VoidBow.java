...
public class VoidBow extends ItemBow {
	...
	public VoidBow(final int uses, final int damage, final Item arrow) {
		this(uses, damage, 72000, arrow);
		setFull3D();
	}

	public VoidBow(final int uses, final int damage, final int maxUseDuraction, final Item arrow) {
		setMaxDamage(uses);
		this.arrow = arrow;
		this.damage = damage;
	...
	public void registerIcons(final IIconRegister icon) {
		final String prefix = "nevermine:animateditem/bowVoid";
		itemIcon = icon.registerIcon(prefix + "_0");
		IIconArray = new IIcon[VoidBow.texture.length];
		for (int N = 0; N < 4; ++N) {
			IIconArray[N] = icon.registerIcon(prefix + VoidBow.texture[N]);
		}
	}
	...
	public void onPlayerStoppedUsing(final ItemStack item, final World par2World, final EntityPlayer par3EntityPlayer, final int par4) {
		...
		if (flag || par3EntityPlayer.inventory.hasItem(arrow)) {
			...
			if (sound(item, par2World, par3EntityPlayer) != null) {
				par2World.playSoundAtEntity(par3EntityPlayer, sound(item, par2World, par3EntityPlayer), 1.0f, 1.0f / (VoidBow.itemRand.nextFloat() * 0.4f + 1.2f) + f * 0.5f);
			}
			if (flag) {
