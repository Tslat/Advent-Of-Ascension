...
public class JusticeBow extends ItemBow {
	...
	public JusticeBow(int uses, int damage, Item arrow) {
		this(uses, damage, 72000, arrow);
		setFull3D();
	}

	public JusticeBow(int uses, int damage, int maxUseDuraction, Item arrow) {
		setMaxDamage(uses);
		this.arrow = arrow;
		this.damage = damage;
	...
	public void registerIcons(IIconRegister icon) {
		String prefix = "nevermine:animateditem/bowJustice";
	...
	public void onPlayerStoppedUsing(ItemStack item, World par2World, EntityPlayer par3EntityPlayer, int par4) {
		...
		if ((flag) || (par3EntityPlayer.inventory.hasItem(arrow))) {
			...
			entityarrow = new EntityElementalArrow(par2World, par3EntityPlayer, f * 2.0F, 20, false);
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean bool) {
		...
		list.add(StringUtil.getColourLocaleString("item.JusticeBow.desc.1", EnumChatFormatting.DARK_GREEN));
