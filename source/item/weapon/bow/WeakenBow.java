...
public class WeakenBow extends BaseBow {
	...
	public void doImpactEffect(EntityHollyArrow arrow, Entity target, Entity shooter) {
		if (target instanceof EntityLivingBase) {
			if (arrow instanceof EntityTippedHollyArrow && shooter instanceof EntityLivingBase && ((EntityTippedHollyArrow)arrow).getBasePotionEffect() == MobEffects.WEAKNESS) {
				if (!arrow.world.isRemote) {
					EntityPotion pot = new EntityPotion(arrow.world, (EntityLivingBase)shooter, PotionUtils.addPotionToItemStack(new ItemStack(Items.SPLASH_POTION), PotionTypes.WEAKNESS));
			...
			((EntityLivingBase)target).addPotionEffect(new PotionEffect(MobEffects.WEAKNESS, 40, 2));
		}
	}
