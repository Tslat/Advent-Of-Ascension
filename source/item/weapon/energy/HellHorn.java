...
public class HellHorn extends BaseEnergy {
	private final float dmg = 16.0f;

	public HellHorn(final int consumeChance, final String effect, final int uses, final int fireRate, final int cost) {
	...
	public void fireAncient(final World world, final ItemStack stack, final EntityPlayer player) {
		world.spawnEntityInWorld(new EntityHellBubbleShot(world, player, dmg));
	}

	@SideOnly(Side.CLIENT)
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		list.add(StringUtil.getColourLocaleStringWithArguments("items.description.damage.ranged", EnumChatFormatting.DARK_RED, Integer.toString((int)dmg)));
		list.add(StringUtil.getColourLocaleString("item.HellHorn.desc.1", EnumChatFormatting.DARK_GREEN));
