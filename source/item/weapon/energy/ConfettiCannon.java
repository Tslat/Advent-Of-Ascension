...
public class ConfettiCannon extends BaseEnergy {

	public ConfettiCannon(final int consumeChance, final String effect, final int uses, final int fireRate, final int cost) {
	...
	public void fireAncient(final World world, final ItemStack stack, final EntityPlayer player) {
		player.worldObj.spawnEntityInWorld(new EntityConfetti(player.worldObj, player, false));
	}

	@SideOnly(Side.CLIENT)
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		list.add(StringUtil.getColourLocaleString("item.ConfettiCannon.desc.1", EnumChatFormatting.DARK_GREEN));
