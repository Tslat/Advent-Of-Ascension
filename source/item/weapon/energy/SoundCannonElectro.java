...
public class SoundCannonElectro extends BaseEnergy {
	private final float dmg = 12.0f;

	public SoundCannonElectro(final int consumeChance, final String effect, final int uses, final int fireRate, final int cost) {
	...
	public void fireAncient(final World world, final ItemStack stack, final EntityPlayer player) {
		switch (nevermine.rand.nextInt(5)) {
			case 0:
				player.worldObj.playSoundAtEntity(player, "nevermine:SoundCannonElectro1", 1.0f, 1.0f);

				if (!player.worldObj.isRemote)
					player.worldObj.spawnEntityInWorld(new EntitySoundCannon1(player.worldObj, player, dmg));
				break;
			case 1:
				player.worldObj.playSoundAtEntity(player, "nevermine:SoundCannonElectro2", 1.0f, 1.0f);

				if (!player.worldObj.isRemote)
					player.worldObj.spawnEntityInWorld(new EntitySoundCannon2(player.worldObj, player, dmg));
				break;
			case 2:
				player.worldObj.playSoundAtEntity(player, "nevermine:SoundCannonElectro3", 1.0f, 1.0f);

				if (!player.worldObj.isRemote)
					player.worldObj.spawnEntityInWorld(new EntitySoundCannon3(player.worldObj, player, dmg));
				break;
			case 3:
				player.worldObj.playSoundAtEntity(player, "nevermine:SoundCannonElectro4", 1.0f, 1.0f);

				if (!player.worldObj.isRemote)
					player.worldObj.spawnEntityInWorld(new EntitySoundCannon4(player.worldObj, player, dmg));
				break;
			case 4:
				player.worldObj.playSoundAtEntity(player, "nevermine:SoundCannonElectro5", 1.0f, 1.0f);
