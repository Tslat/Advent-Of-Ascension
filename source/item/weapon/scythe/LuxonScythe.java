...
public class LuxonScythe extends BaseScythe {
	private float dmg;

	public LuxonScythe(final int uses, final float dealDamage) {
	...
	public void fireScythe(final World world, final ItemStack stack, final EntityPlayer player) {
		player.worldObj.spawnEntityInWorld(new EntityScythe(player.worldObj, player, dmg, 3));
	}
	...
	public void addInformation(final ItemStack item, final EntityPlayer player, final List list, final boolean bool) {
		...
		list.add(StringUtil.getColourLocaleString("item.LuxonScythe.desc.1", EnumChatFormatting.DARK_GREEN));
