...
public class BoulderBomber extends BaseGun {
	private final float dmg = 35.0f;

	public BoulderBomber(final int consumeChance, final String effect, final int uses, final int fireRate, final Item item) {
	...
	public void fireGun(final World world, final ItemStack stack, final EntityPlayer player, final float multi, boolean consume) {
		player.worldObj.spawnEntityInWorld(new EntityCannonBall(player.worldObj, player, dmg * multi, 5, 0.06f));
		player.worldObj.spawnEntityInWorld(new EntityCannonBall(player.worldObj, player, dmg * multi, 5, 0.03f));
	}

	@SideOnly(Side.CLIENT)
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		list.add(StringUtil.getColourLocaleStringWithArguments("items.description.damage.ranged", EnumChatFormatting.DARK_RED, Integer.toString((int)dmg)));
		list.add(StringUtil.getColourLocaleString("item.BoulderBomber.desc.1", EnumChatFormatting.DARK_GREEN));
