...
public class MissileMaker extends BaseCannon {
	...
	public MissileMaker(double dmg, int durability, int firingDelayTicks, float recoil) {
		...
		setTranslationKey("MissileMaker");
	...
	public SoundEvent getFiringSound() {
		return SoundsRegister.MISSILE_MAKER_FIRE;
	}
	...
	public BaseBullet findAndConsumeAmmo(EntityPlayer player, ItemStack gunStack, EnumHand hand) {
		if (ItemUtil.findInventoryItem(player, new ItemStack(WeaponRegister.GRENADE), true, 1 + EnchantmentHelper.getEnchantmentLevel(EnchantmentsRegister.GREED, gunStack)))
			return new EntityMultiplyingGrenade(player, (BaseGun)gunStack.getItem(), hand, 120, 0);
	...
	public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag flag) {
		tooltip.add(ItemUtil.getFormattedDescriptionText("item.MissileMaker.desc.1", Enums.ItemDescriptionType.POSITIVE));
