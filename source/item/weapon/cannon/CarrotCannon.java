...
public class CarrotCannon extends BaseCannon {
	...
	public CarrotCannon(double dmg, int durability, int firingDelayTicks, float recoil) {
		...
		setTranslationKey("CarrotCannon");
	...
	public BaseBullet findAndConsumeAmmo(EntityPlayer player, ItemStack gunStack, EnumHand hand) {
		if (ItemUtil.findInventoryItem(player, new ItemStack(Items.CARROT), true, 1 + EnchantmentHelper.getEnchantmentLevel(EnchantmentsRegister.GREED, gunStack)))
			return new EntityCarrotBall(player, (BaseGun)gunStack.getItem(), hand, 120, 0);
	...
	public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag flag) {
		...
		tooltip.add(ItemUtil.getFormattedDescriptionText("items.description.ammo.other", Enums.ItemDescriptionType.ITEM_AMMO_COST, StringUtil.getLocaleString("item.carrots.name")));
	}
}
