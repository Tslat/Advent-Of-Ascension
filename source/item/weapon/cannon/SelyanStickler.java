...
public class SelyanStickler extends BaseCannon {
	...
	public SelyanStickler(double dmg, int durability, int firingDelayTicks, float recoil) {
		...
		setTranslationKey("SelyanStickler");
	...
	public BaseBullet findAndConsumeAmmo(EntityPlayer player, ItemStack gunStack, EnumHand hand) {
		if (ItemUtil.findInventoryItem(player, new ItemStack(WeaponRegister.GRENADE), true, 1 + EnchantmentHelper.getEnchantmentLevel(EnchantmentsRegister.GREED, gunStack)))
			return new EntitySelyanSticklerShot(player, (BaseGun)gunStack.getItem(), hand, 120, 0);
	...
	public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag flag) {
		...
		tooltip.add(ItemUtil.getFormattedDescriptionText("item.SelyanStickler.desc.1", Enums.ItemDescriptionType.POSITIVE));
