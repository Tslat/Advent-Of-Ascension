	...
	public BaseBullet findAndConsumeAmmo(EntityPlayer player, ItemStack gunStack, EnumHand hand) {
		if (ItemUtil.findInventoryItem(player, new ItemStack(ItemRegister.CANNONBALL), true, 1 + EnchantmentHelper.getEnchantmentLevel(EnchantmentsRegister.GREED, gunStack))) {
			if (!player.world.isRemote)
				player.world.spawnEntity(new EntityGoldenCannonball(player, (BaseGun)gunStack.getItem(), hand, 120, 0, 0, 0.325f, 0));

			return new EntityGoldenCannonball(player, (BaseGun)gunStack.getItem(), hand, 120, 0, 0, 0, 0);
		}
