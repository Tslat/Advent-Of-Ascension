...
public class LunarStaff extends BaseStaff {
	...
	public boolean checkConditions(final World world, final ItemStack stack, final EntityPlayer player) {
		if (stack.getItem() == Weaponizer.LunarStaff) {
	...
	public void fireStaff(final World world, final ItemStack stack, final EntityPlayer player) {
		if (!world.isRemote) {
			world.spawnEntityInWorld(new EntityLunarFall(world, posX, posY, posZ, player));
			player.worldObj.playSoundAtEntity(player, "nevermine:LunarStaff", 1.0f, 1.0f);
		}
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		...
		list.add(StringUtil.getColourLocaleString("item.LunarStaff.desc.1", EnumChatFormatting.DARK_GREEN));
