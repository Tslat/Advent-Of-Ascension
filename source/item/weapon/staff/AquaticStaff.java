...
public class AquaticStaff extends BaseStaff {
	...
	public void fireStaff(final World world, final ItemStack stack, final EntityPlayer player) {
		...
		if (!player.worldObj.isRemote) {
			player.worldObj.spawnEntityInWorld(new EntityStaffAquatic(player.worldObj, player, dmg, 0, 0.0f));
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		...
		list.add(StringUtil.getColourLocaleString("item.AquaticStaff.desc.1", EnumChatFormatting.DARK_GREEN));
		...
		list.add(StringUtil.getColourLocaleStringWithArguments("items.description.runes.specific", EnumChatFormatting.WHITE, Integer.toString(4), StringUtil.getLocaleString("item.WaterRune.name")));
	}

	static {
		runes.put((ItemRune)Itemizer.WindRune, 2);
		runes.put((ItemRune)Itemizer.WaterRune, 4);
	}
}
