...
public class MeteorStaff extends BaseStaff {
	...
	public boolean checkConditions(final World world, final ItemStack stack, final EntityPlayer player) {
		if (stack.getItem() == Weaponizer.MeteorStaff) {
	...
	public void fireStaff(final World world, final ItemStack stack, final EntityPlayer player) {
		if (!world.isRemote) {
			for (int i = 0; i < 8; ++i) {
				world.spawnEntityInWorld(new EntityMeteorFall(world, posX, posY, posZ, player));
			}

			player.worldObj.playSoundAtEntity(player, "nevermine:MeteorStaff", 1.0f, 1.0f);
		}
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		list.add(StringUtil.getColourLocaleString("item.MeteorStaff.desc.1", EnumChatFormatting.DARK_GREEN));
		...
		list.add(StringUtil.getColourLocaleStringWithArguments("items.description.runes.specific", EnumChatFormatting.WHITE, Integer.toString(3), StringUtil.getLocaleString("item.StormRune.name")));
	}

	static {
		runes.put((ItemRune)Itemizer.WindRune, 3);
		runes.put((ItemRune)Itemizer.StormRune, 3);
	}
}
