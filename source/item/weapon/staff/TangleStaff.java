...
public class TangleStaff extends BaseStaff {
	...
	public boolean checkConditions(final World world, final ItemStack stack, final EntityPlayer player) {
		if (stack.getItem() == Weaponizer.TangleStaff) {
	...
	public void fireStaff(final World world, final ItemStack stack, final EntityPlayer player) {
		if (!world.isRemote) {
			player.worldObj.playSoundAtEntity(player, "nevermine:TangleStaff", 1.0f, 1.0f);
			for (int i = 0; i < 8; ++i) {
				world.spawnEntityInWorld(new EntityTangleFall(world, posX, posY, posZ, player));
			}
		}
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		list.add(StringUtil.getColourLocaleString("item.TangleStaff.desc.1", EnumChatFormatting.DARK_GREEN));
