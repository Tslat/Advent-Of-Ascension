...
public class WaterStaff extends BaseStaff {
	...
	public void fireStaff(final World world, final ItemStack stack, final EntityPlayer player) {
		...
		if (!player.worldObj.isRemote) {
			player.worldObj.spawnEntityInWorld(new EntityStaffWater(player.worldObj, player, dmg, 0, 0.0f));
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		...
		list.add(StringUtil.getColourLocaleStringWithArguments("items.description.runes.specific", EnumChatFormatting.WHITE, Integer.toString(2), StringUtil.getLocaleString("item.WaterRune.name")));
	}

	static {
		runes.put((ItemRune)Itemizer.WindRune, 2);
		runes.put((ItemRune)Itemizer.WaterRune, 2);
	}
}
