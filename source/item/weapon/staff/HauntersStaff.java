...
public class HauntersStaff extends BaseStaff {
	...
	public void fireStaff(final World world, final ItemStack stack, final EntityPlayer player) {
		...
		if (!player.worldObj.isRemote) {
			player.worldObj.spawnEntityInWorld(new EntityHauntersShot(player.worldObj, player, 16.0f, 0, 0.0f));
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		...
		list.add(StringUtil.getColourLocaleString("item.HauntersStaff.desc.1", EnumChatFormatting.DARK_GREEN));
