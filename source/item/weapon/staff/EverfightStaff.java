...
public class EverfightStaff extends BaseStaff {
	...
	public void fireStaff(final World world, final ItemStack stack, final EntityPlayer player) {
		...
		player.addPotionEffect(new PotionEffect(Potion.resistance.id, 100, size));
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		list.add(StringUtil.getColourLocaleString("item.EverfightStaff.desc.1", EnumChatFormatting.DARK_GREEN));
		list.add(StringUtil.getColourLocaleString("item.EverfightStaff.desc.2", EnumChatFormatting.DARK_GREEN));
		list.add(StringUtil.getColourLocaleString("item.EverfightStaff.desc.3", EnumChatFormatting.DARK_GREEN));
		...
		list.add(StringUtil.getColourLocaleStringWithArguments("items.description.runes.specific", EnumChatFormatting.WHITE, Integer.toString(5), StringUtil.getLocaleString("item.EnergyRune.name")));
	}

	static {
		runes.put((ItemRune)Itemizer.CompassRune, 5);
		runes.put((ItemRune)Itemizer.EnergyRune, 5);
	}
}
