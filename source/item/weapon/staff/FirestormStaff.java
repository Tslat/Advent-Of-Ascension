...
public class FirestormStaff extends BaseStaff {
	...
	public boolean checkConditions(final World world, final ItemStack stack, final EntityPlayer player) {
		if (stack.getItem() == Weaponizer.FirestormStaff) {
	...
	public void fireStaff(final World world, final ItemStack stack, final EntityPlayer player) {
		if (!world.isRemote) {
			for (int i = 0; i < 8; ++i) {
				world.spawnEntityInWorld(new EntityFireFall(world, posX, posY, posZ, player));
			}
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		list.add(StringUtil.getColourLocaleString("item.FirestormStaff.desc.1", EnumChatFormatting.DARK_GREEN));
		...
		list.add(StringUtil.getColourLocaleStringWithArguments("items.description.runes.specific", EnumChatFormatting.WHITE, Integer.toString(2), StringUtil.getLocaleString("item.FireRune.name")));
	...
	static {
		...
		runes.put((ItemRune)Itemizer.FireRune, 2);
