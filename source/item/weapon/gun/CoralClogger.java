...
public class CoralClogger extends BaseGun {
	...
	public BaseBullet findAndConsumeAmmo(PlayerEntity player, ItemStack gunStack, Hand hand) {
		if (ItemUtil.findInventoryItem(player, new ItemStack(AoAItems.METAL_SLUG.get()), true, 1 + EnchantmentHelper.getEnchantmentLevel(AoAEnchantments.GREED.get(), gunStack)))
			return new MetalSlugEntity(player, (BaseGun)gunStack.getItem(), hand, 120, 0);
	...
	public void doImpactDamage(Entity target, LivingEntity shooter, BaseBullet bullet, float bulletDmgMultiplier) {
		...
		if (target instanceof LivingEntity) {
			DamageUtil.doScaledKnockback((LivingEntity)target, shooter, 1.143f, shooter.getPosX() - target.getPosX(), shooter.getPosZ() - target.getPosZ());
