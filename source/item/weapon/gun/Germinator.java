...
import net.tslat.aoa3.entity.projectile.gun.GreenBulletEntity;
...
public class Germinator extends BaseGun {
	...
	public BaseBullet findAndConsumeAmmo(PlayerEntity player, ItemStack gunStack, Hand hand) {
		if (ItemUtil.findInventoryItem(player, new ItemStack(AoAItems.LIMONITE_BULLET.get()), true, 1 + EnchantmentHelper.getEnchantmentLevel(AoAEnchantments.GREED.get(), gunStack)))
			return new GreenBulletEntity(player, (BaseGun)gunStack.getItem(), hand, 120, 0);
	...
	protected void doImpactEffect(Entity target, LivingEntity shooter, BaseBullet bullet, float bulletDmgMultiplier) {
		if (target instanceof LivingEntity)
			EntityUtil.applyPotions(target, new PotionUtil.EffectBuilder(Effects.POISON, 50).level(2));
	}
