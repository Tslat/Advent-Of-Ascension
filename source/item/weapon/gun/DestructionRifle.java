...
public class DestructionRifle extends BaseGun {
	...
	public DestructionRifle(final int consumeChance, final String effect, final int uses, final int fireRate, final Item item) {
	...
	public void fireGun(final World world, final ItemStack stack, final EntityPlayer player, final float multi, boolean consume) {
		if (nevermine.rand.nextInt(3) == 2) {
			...
			player.worldObj.spawnEntityInWorld(new EntityMetalPellet(player.worldObj, player, 16.0f * multi, 0));
		}
	}
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean bool) {
		...
		list.add(StringUtil.getColourLocaleString("item.DestructionRifle.desc.1", EnumChatFormatting.DARK_GREEN));
		list.add(StringUtil.getColourLocaleString("item.DestructionRifle.desc.2", EnumChatFormatting.DARK_GREEN));
