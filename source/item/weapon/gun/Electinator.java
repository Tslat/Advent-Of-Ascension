...
public class Electinator extends BaseGun {
	...
	public Electinator(final int consumeChance, final String effect, final int uses, final int fireRate, final Item item) {
	...
	public void fireGun(final World world, final ItemStack stack, final EntityPlayer player, final float multi, boolean consume) {
		player.worldObj.spawnEntityInWorld(new EntityGolemGunBullet(player.worldObj, player, dmg * multi, 1));
	}
