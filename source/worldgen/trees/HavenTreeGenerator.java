...
public class HavenTreeGenerator extends TreeGenerator {
	...
	public HavenTreeGenerator(@Nullable SaplingBlock sapling, @Nullable LeavesBlock leaves, Random rand) {
		...
		if (leaves == null) {
			switch (rand.nextInt(6)) {
				case 0:
					leavesBlock = AoABlocks.BLUE_HAVEN_LEAVES.get().getDefaultState();
					break;
				case 1:
					leavesBlock = AoABlocks.PINK_HAVEN_LEAVES.get().getDefaultState();
					break;
				case 2:
					leavesBlock = AoABlocks.PURPLE_HAVEN_LEAVES.get().getDefaultState();
					break;
				case 3:
					leavesBlock = AoABlocks.RED_HAVEN_LEAVES.get().getDefaultState();
					break;
				case 4:
					leavesBlock = AoABlocks.TURQUOISE_HAVEN_LEAVES.get().getDefaultState();
					break;
				case 5:
				default:
					leavesBlock = AoABlocks.YELLOW_HAVEN_LEAVES.get().getDefaultState();
					break;
			}
		}
		else {
	...
	private boolean generateTree1(IWorld world, Random rand, BlockPos pos) {
		...
		for (int i = 0; i < trunkHeight; i++) {
			...
			if (!builtLeafRing && i > 1 && (i >= trunkHeight - 4 || rand.nextInt(3) == 0)) {
				...
				placeBlock(world, leafPosBase.offset(Direction.NORTH), leavesBlock);
				placeBlock(world, leafPosBase.offset(Direction.EAST), leavesBlock);
				placeBlock(world, leafPosBase.offset(Direction.SOUTH), leavesBlock);
				placeBlock(world, leafPosBase.offset(Direction.WEST), leavesBlock);
	...
	private boolean generateTree3(IWorld world, Random rand, BlockPos pos) {
		...
		BlockPos.Mutable movablePos = new BlockPos.Mutable(pos.down());
		BlockState log = Blocks.OAK_LOG.getDefaultState();
		BlockState barkLog = Blocks.OAK_WOOD.getDefaultState();
		...
		for (Direction direction : Direction.Plane.HORIZONTAL) {
			branchMovablePos = new BlockPos.Mutable(movablePos);
