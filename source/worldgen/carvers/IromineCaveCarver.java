...
public class IromineCaveCarver extends CaveWorldCarver {
	public IromineCaveCarver(Function<Dynamic<?>, ? extends ProbabilityConfig> config, int maxHeight) {
	...
	protected boolean canCarveBlock(BlockState state, BlockState aboveState) {
		...
		return (block == AoABlocks.IRO_GRASS.get() || block == AoABlocks.IROSTONE.get()) && !aboveState.getFluidState().isTagged(FluidTags.WATER);
	}
}
