...
public class CandylandDimension extends Dimension {
	public CandylandDimension(World world, DimensionType dimType) {
	...
	public ChunkGenerator<? extends GenerationSettings> createChunkGenerator() {
		...
		BiomeProvider biomeProvider = biomeProviderType.create(biomeProviderType.createSettings(world.getWorldInfo()).setBiome(AoABiomes.CANDYLAND.get()));
		BlockState fillerBlock = AoABlocks.CANDIED_DIRT.get().getDefaultState();
		BlockState liquidBlock = AoABlocks.CANDIED_WATER.get().getDefaultState();
	...
	public Vec3d getFogColor(float celestialAngle, float partialTicks) {
		float f2 = MathHelper.clamp(MathHelper.sin(celestialAngle * (float)Math.PI * 2.0F) * 2.0F + 0.5F, 1.5f, 5f);
		float f3 = 1.7F * f2;
		float f4 = 0.5F * f2;
		float f5 = 0.5F * f2;
