...
public class TinyBlueMushroom extends AoAStructure {
	...
	protected void build(IWorld world, Random rand, BlockPos basePos) {
		...
		if (rand.nextInt(3) == 0) {
			...
			world.setBlockState(basePos.up(2), AoABlocks.BLUE_SHROOM.get().getDefaultState(), 2);
		}
		else if (rand.nextBoolean()){
			world.setBlockState(basePos, AoABlocks.SHROOM_STEM.get().getDefaultState(), 2);
			world.setBlockState(basePos.up(), AoABlocks.BLUE_SHROOM.get().getDefaultState(), 2);
		}
		else {
			world.setBlockState(basePos, AoABlocks.BLUE_SHROOM.get().getDefaultState(), 2);
		}
	}
}
