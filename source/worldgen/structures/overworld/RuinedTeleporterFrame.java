package net.tslat.aoa3.worldgen.structures.overworld;
...
public class RuinedTeleporterFrame extends AoAStructure { //StructureSize: 11x9x15
	...
	private static final BlockState ancientRock = AoABlocks.ANCIENT_ROCK.get().getDefaultState();
	private static final BlockState carvedRuneOfDirection = AoABlocks.CARVED_RUNE_OF_DIRECTION.get().getDefaultState();
	private static final BlockState carvedRuneOfSpace = AoABlocks.CARVED_RUNE_OF_SPACE.get().getDefaultState();
	private static final BlockState carvedRuneOfReality = AoABlocks.CARVED_RUNE_OF_REALITY.get().getDefaultState();
	private static final BlockState carvedRuneOfTravel = AoABlocks.CARVED_RUNE_OF_TRAVEL.get().getDefaultState();
	...
	protected void spawnEntities(IWorld world, Random rand, BlockPos basePos) {
		CorruptedTravellerEntity corruptedTraveller = new CorruptedTravellerEntity(AoAEntities.NPCs.CORRUPTED_TRAVELLER.get(), world.getWorld());

		corruptedTraveller.setLocationAndAngles(basePos.getX() + 5, basePos.getY() + 4, basePos.getZ() + 7, rand.nextFloat() * 360, 0);
		world.addEntity(corruptedTraveller);
	}
}
