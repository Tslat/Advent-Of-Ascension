...
public class InjectOrReplaceLootModifier extends LootModifier {
	...
	public InjectOrReplaceLootModifier(ILootCondition[] conditions, ResourceLocation[] additionalTables) {
		super(conditions);
	...
	protected List<ItemStack> doApply(List<ItemStack> generatedLoot, LootContext context) {
		...
		for (ResourceLocation tableLocation : additionalTables) {
			...
			if (compatible && table != LootTable.EMPTY)
				table.getRandomItems(context, newLoot::add);
		}
	...
	public static class Serializer extends GlobalLootModifierSerializer<InjectOrReplaceLootModifier> {
		@Override
		public InjectOrReplaceLootModifier read(ResourceLocation location, JsonObject object, ILootCondition[] lootConditions) {
			...
			return new InjectOrReplaceLootModifier(lootConditions, tableList);
		}
