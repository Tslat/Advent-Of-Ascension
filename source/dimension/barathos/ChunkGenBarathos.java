...
public class ChunkGenBarathos implements IChunkGenerator {
	...
	private CaveGenBarathos caveGen = new CaveGenBarathos();

	protected ChunkGenBarathos(World world) {
		this.world = world;
	...
	private void replaceBiomeBlocks() {
		...
		for (int x = 0; x < 13 + rand.nextInt(4); ++x) {
			for (int z = 0; z < 13 + rand.nextInt(4); ++z) {
	...
	public void populate(int chunkX, int chunkZ) {
		...
		if (ConfigurationUtil.StructureConfig.barathos.baronessArenaSpawnChance > 0 && rand.nextInt(ConfigurationUtil.StructureConfig.barathos.baronessArenaSpawnChance) == 0) {
			...
			if (world.getBlockState(pos.setPos(x + 10, y - 1, z + 9)) == biome.topBlock)
		...
		if (ConfigurationUtil.StructureConfig.barathos.powerRuneShrineSpawnChance > 0 && rand.nextInt(ConfigurationUtil.StructureConfig.barathos.powerRuneShrineSpawnChance) == 0) {
			...
			StructuresHandler.generateStructure("PowerRuneShrine", world, rand, pos.setPos(x, y, z));
		}
