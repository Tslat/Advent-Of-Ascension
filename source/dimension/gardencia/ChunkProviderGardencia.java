...
public class ChunkProviderGardencia implements IChunkProvider {
	...
	private WorldGenerator flowertree = new Flowertree();
	private WorldGenerator starflowerblue1 = new StarflowerBlue1();
	private WorldGenerator starflowerblue2 = new StarflowerBlue2();
	private WorldGenerator starflowerrose1 = new StarflowerRose1();
	private WorldGenerator starflowerrose2 = new StarflowerRose2();
	private WorldGenerator sunflower1 = new Sunflower1();
	private WorldGenerator sunflower2 = new Sunflower2();
	private WorldGenerator tulipblue = new TulipBlue();
	private WorldGenerator tuliplightpurple = new TulipLightPurple();
	private WorldGenerator tulippurple = new TulipPurple();
	private WorldGenerator gardengrass = new GardenGrass();

	public ChunkProviderGardencia(World par1World, long par2) {
		worldObj = par1World;
					...
					for (int l2 = 0; l2 < 8; ++l2) {
						...
						for (int i3 = 0; i3 < 4; ++i3) {
							...
							for (int k3 = 0; k3 < 4; ++k3) {
								if ((d15 += d16) > 0.0D)
									b[j3 += short1] = Blockizer.StoneGardencia;

								else if (k2 * 8 + l2 < b0)
									b[j3 += short1] = Blockizer.GrassGardencia;
								else
									b[j3 += short1] = null;
		...
		for (int l1 = 255; l1 >= 0; --l1) {
			...
			if (l1 <= 0 + p_150560_2_.nextInt(5)) {
				...
				if (block2 != null && block2.getMaterial() != Material.air) {
					if (block2 == Blockizer.StoneGardencia) {
						if (k == -1) {
							if (l <= 0) {
								block = null;
								b0 = 0;
								block1 = Blockizer.StoneGardencia;
							}
							else if (l1 >= 59 && l1 <= 64) {
								block = b.topBlock;
							...
							if (l1 < 63 && (block == null || block.getMaterial() == Material.air)) {
								if (b.getFloatTemperature(p_150560_5_, l1, p_150560_6_) < 0.15F) {
									...
									block = Blockizer.StoneGardencia;
									b0 = 0;
								}
							}
							...
							if (l1 >= 62) {
								...
								block = null;
								block1 = Blockizer.StoneGardencia;
								p_150560_3_[i2] = Blockizer.StoneGardencia;
							}
							else {
							...
							if (k == 0 && block1 == Blockizer.StoneGardencia) {
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		for (i = 0; i < 2; i++) {
			if (TerrainGen.populate(par1IChunkProvider, worldObj, rand, var4, var5, false, LAKE)) {
				var12 = var4 + rand.nextInt(16) + 8;
				var14 = var5 + rand.nextInt(16) + 8;
				var13 = worldObj.getHeightValue(var12, var14);
				lakegen.generate(worldObj, rand, var12, var13, var14);
			}
		}
		if (biomegenbase == DimensionOrganizer.Gardencia) {
			for (i = 0; i < 2; i++) {
				...
				if (y > 67) {
					...
					if (rand.nextInt(2) == 1) {
						if (randomNum == 9) {
							flowertree.generate(worldObj, rand, x, y, z);
						}
						else if (randomNum == 8) {
							starflowerblue1.generate(worldObj, rand, x, y, z);
						}
						else if (randomNum == 7) {
							starflowerblue2.generate(worldObj, rand, x, y, z);
						}
						else if (randomNum == 6) {
							starflowerrose1.generate(worldObj, rand, x, y, z);
						}
						else if (randomNum == 5) {
							starflowerrose2.generate(worldObj, rand, x, y, z);
						}
						else if (randomNum == 4) {
							sunflower1.generate(worldObj, rand, x, y, z);
						}
						else if (randomNum == 3) {
							sunflower2.generate(worldObj, rand, x, y, z);
						}
						else if (randomNum == 2) {
							tulipblue.generate(worldObj, rand, x, y, z);
						}
						else if (randomNum == 1) {
							tuliplightpurple.generate(worldObj, rand, x, y, z);
						}
						else {
							tulippurple.generate(worldObj, rand, x, y, z);
						}
					}
				}
			}
			...
			for (i = 0; i < 59; i++) {
				...
				if (worldObj.getBlock(x, y, z) == Blocks.air)
					if (y > 67) {
						...
						if (pick == 2 || pick == 3) {
							worldObj.setBlock(x, y, z, Blockizer.Daileers);
						}
						else if (pick == 1 || pick == 4) {
							worldObj.setBlock(x, y, z, Blockizer.Lylips);
						}
						else {
							gardengrass.generate(worldObj, rand, x, y, z);
						}
					}
			}
		}
		else if (biomegenbase == DimensionOrganizer.GardenciaFungal) {
			x = var4 + rand.nextInt(16);
			z = var5 + rand.nextInt(16);
			...
			if (y > 67) {
				...
				if (randomNum == 1) {
					new GardenFungi2().generate(worldObj, rand, x, y, z);
				}
				else if (randomNum == 2) {
					new GardenFungi1().generate(worldObj, rand, x, y, z);
				}
				else {
					new GardenFungi3().generate(worldObj, rand, x - 1, y, z - 1);
				}
			}
			...
			for (i = 0; i < 60; i++) {
				...
				if (worldObj.getBlock(x, y, z) == Blocks.air)
					if (y > 67) {
						...
						if (pick == 2) {
							worldObj.setBlock(x, y, z, Blockizer.OrangeFungi);
						}
						else if (pick == 1) {
							worldObj.setBlock(x, y, z, Blockizer.AquaFungi);
						}
						else {
							worldObj.setBlock(x, y, z, Blockizer.GardenGrass);
						}
					}
			}
		}
			...
			for (i = 0; i < 4; i++) {
				...
				if (y > 67) {
					if (worldObj.getBlock(x + 1, y - 1, z + 1) == Blockizer.GrassGardencia) {
						if (rand.nextInt(4) != 1) {
							int randomNum = rand.nextInt(3);
							if (randomNum == 1) {
								new MarshLilly3().generate(worldObj, rand, x, y, z);
							}
							else if (randomNum == 2) {
								new MarshLilly1().generate(worldObj, rand, x, y, z);
							}
							else {
								new MarshLilly2().generate(worldObj, rand, x, y, z);
							}
						}
						else {
							if (rand.nextInt(2) == 1) {
								new MarshFern1().generate(worldObj, rand, x - 2, y, z - 2);
							}
							else {
								new MarshFern2().generate(worldObj, rand, x - 1, y, z - 1);
							}
						}
					}
				}
		...
		if (rand.nextInt(650) == 127) {
			...
			(new RuneShrinePlatform(7)).generate(worldObj, rand, x, y, z);
		}

		if (rand.nextInt(300) == 144) {
			x = var4 + rand.nextInt(16);
			z = var5 + rand.nextInt(16);
			y = worldObj.getHeightValue(x + 1, z + 1);
			if (worldObj.getBlock(x, y - 1, z) == Blockizer.GrassGardencia)
				(new WizardFlower()).generate(worldObj, rand, x, y - 1, z);
		}

		if (rand.nextInt(300) == 265) {
			x = var4 + rand.nextInt(16);
			z = var5 + rand.nextInt(16);
			y = worldObj.getHeightValue(x + 6, z + 6);
			if (worldObj.getBlock(x, y - 1, z) == Blockizer.GrassGardencia)
				(new DayseeFlower()).generate(worldObj, rand, x, y - 1, z);
		}

		if (rand.nextInt(500) == 223) {
			x = var4 + rand.nextInt(16);
			z = var5 + rand.nextInt(16);
			y = worldObj.getHeightValue(x + 1, z + 1);
			if (worldObj.getBlock(x, y - 1, z) == Blockizer.GrassGardencia)
				(new LottoGardenciaStructure()).generate(worldObj, rand, x, y - 1, z);
		}
