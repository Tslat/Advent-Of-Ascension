...
public class TeleporterHaven extends Teleporter {
	...
	public void placeInPortal(final Entity par1Entity, final double par2, final double par4, final double par6, final float par8) {
		if (par1Entity instanceof EntityPlayer) {
			if (worldServerInstance.provider.dimensionId == ConfigurationHelper.haven) {
	...
	public boolean placeInExistingPortal(final Entity par1Entity, final double par2, final double par4, final double par6, final float par8) {
			...
			for (int l2 = l - short1; l2 <= l + short1; ++l2) {
				...
				for (int l3 = i2 - short1; l3 <= i2 + short1; ++l3) {
					...
					for (int i3 = worldServerInstance.getActualHeight() - 1; i3 >= 0; --i3) {
						if (worldServerInstance.getBlock(l2, i3, l3) == Blockizer.havenPortal) {
							while (worldServerInstance.getBlock(l2, i3 - 1, l3) == Blockizer.havenPortal) {
								--i3;
							}
		...
		if (d3 >= 0.0) {
			...
			double d6 = k + 0.5;
			int i4 = -1;
			if (worldServerInstance.getBlock(i - 1, j, k) == Blockizer.havenPortal) {
				i4 = 2;
			}
			if (worldServerInstance.getBlock(i + 1, j, k) == Blockizer.havenPortal) {
				i4 = 0;
			}
			if (worldServerInstance.getBlock(i, j, k - 1) == Blockizer.havenPortal) {
				i4 = 3;
			}
			if (worldServerInstance.getBlock(i, j, k + 1) == Blockizer.havenPortal) {
				i4 = 1;
		...
		for (int i4 = 0; i4 < 4; ++i4) {
			for (int j3 = 0; j3 < 4; ++j3) {
				for (int k4 = -1; k4 < 4; ++k4) {
					...
					worldServerInstance.setBlock(l3, i5, j4, flag ? Blockizer.TwinkleStone : Blockizer.havenPortal, 0, 2);
				}
			}
			for (int j3 = 0; j3 < 4; ++j3) {
				for (int k4 = -1; k4 < 4; ++k4) {
