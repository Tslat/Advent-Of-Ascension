...
public class ChunkGenCreeponia implements IChunkGenerator {
	...
	private CaveGenCreeponia caveGen = new CaveGenCreeponia();

	protected ChunkGenCreeponia(World world) {
		this.world = world;
	...
	private void generateBiomeTerrain(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x, int z, double noiseVal) {
		...
		for (int j1 = 255; j1 >= 0; --j1) {
			if (j1 <= 0) {
				...
				else if (isStoneFill(checkBlock.getBlock())) {
					if (j == -1) {
						if (k <= 0) {
	...
	public void populate(int chunkX, int chunkZ) {
		...
		if (ConfigurationUtil.StructureConfig.creeponia.creeperHQSpawnChance > 0 && rand.nextInt(ConfigurationUtil.StructureConfig.creeponia.creeperHQSpawnChance) == 0) {
			...
			if (world.getBlockState(pos.setPos(x + 6, y - 1, z + 6)) == biome.topBlock)
				StructuresHandler.generateStructure("CreeponiaBank", world, rand, pos.setPos(x, y, z));
		}
