...
public class ChunkGenDeeplands implements IChunkGenerator {
	...
	private final Biome biome = DimensionRegister.biomeDeeplands;
	...
	protected ChunkGenDeeplands(World world) {
		this.world = world;
		this.rand = new Random(world.getSeed());
		this.world.setSeaLevel(4);
						...
						for (int k2 = 0; k2 < 4; ++k2) {
							...
							for (int l2 = 0; l2 < 4; ++l2) {
								if ((lvt_45_1_ += d16) > 0.0D) {
									if (i2 * 8 + j2 < 130)
										primer.setBlockState(i * 4 + k2, i2 * 8 + j2, l * 4 + l2, biome.fillerBlock);
								}
								else if (i2 * 8 + j2 < 20) {
									primer.setBlockState(i * 4 + k2, i2 * 8 + j2, l * 4 + l2, biome.topBlock);
								}
	...
	public void populate(int chunkX, int chunkZ) {
		...
		if (ConfigurationUtil.StructureConfig.deeplands.arocknidCaveSpawnChance > 0 && rand.nextInt(ConfigurationUtil.StructureConfig.deeplands.arocknidCaveSpawnChance) == 0) {
			...
			if (world.getBlockState(pos.setPos(x + 9, y, z + 9)).getBlock() == Blocks.AIR)
				StructuresHandler.generateStructure("DeepLottoShelter", world, rand, pos.setPos(x, y, z));
		}
