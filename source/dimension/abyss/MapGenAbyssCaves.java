...
public class MapGenAbyssCaves extends MapGenBase {
	...
	protected void digBlock(Block[] data, int index, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop) {
		BiomeGenBase biome = worldObj.getBiomeGenForCoords(x + chunkX * 16, z + chunkZ * 16);
		Block top = (isExceptionBiome(biome) ? Blockizer.AbyssGrass : biome.topBlock);
		Block filler = (isExceptionBiome(biome) ? Blockizer.AbyssStone : biome.fillerBlock);
		Block block = data[index];

		if (block == Blockizer.AbyssStone || block == filler || block == top) {
			if (y < 10) {
				data[index] = Blockizer.ToxicBlock;
			}
			else {
				data[index] = null;
