...
import net.nevermine.structures.lborean.*;
...
public class ChunkProviderBorean implements IChunkProvider {
	...
	private WorldGenerator coral1 = new Coral1();
	private WorldGenerator coral2 = new Coral2();
	private WorldGenerator coral3 = new Coral3();
	private WorldGenerator coral4 = new Coral4();
	private WorldGenerator coral5 = new Coral5();
	private WorldGenerator coral6 = new Coral6();

	private WorldGenLakes lakegen = new WorldGenLakes(Blocks.water);

	public ChunkProviderBorean(World par1World, long par2) {
		worldObj = par1World;
					...
					for (int l2 = 0; l2 < 8; ++l2) {
						...
						for (int i3 = 0; i3 < 4; ++i3) {
							...
							for (int k3 = 0; k3 < 4; ++k3) {
								if ((d15 += d16) > 0.0D)
									b[j3 += short1] = Blockizer.StoneBorean;

								else if (k2 * 8 + l2 < b0)
									b[j3 += short1] = Blockizer.GrassBorean;
								else
									b[j3 += short1] = null;
		...
		for (int l1 = 255; l1 >= 0; --l1) {
			...
			if (l1 <= 0 + p_150560_2_.nextInt(5)) {
				...
				if (block2 != null && block2.getMaterial() != Material.air) {
					if (block2 == Blockizer.StoneBorean) {
						if (k == -1) {
							if (l <= 0) {
								block = null;
								b0 = 0;
								block1 = Blockizer.StoneBorean;
							}
							else if (l1 >= 59 && l1 <= 64) {
								block = b.topBlock;
							...
							if (l1 >= 62) {
								...
								block = null;
								block1 = Blockizer.StoneBorean;
								p_150560_3_[i2] = Blockizer.StoneBorean;
							}
							else {
							...
							if (k == 0 && block1 == Blockizer.StoneBorean) {
	...
	public Chunk provideChunk(int par1, int par2) {
		...
		for (int i = 0; i < 65536; i++) {
			if (ablock[i] == Blockizer.GrassBorean && biomesForGeneration[(int)(Math.floor(i / 256))] == DimensionOrganizer.BoreanRed)
				ablock[i] = Blockizer.HardCoral;
		}
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		if (biomegenbase == DimensionOrganizer.Borean) {
			for (i = 0; i < 8; i++) {
			...
			for (i = 0; i < 3; i++) {
				...
				if (randomNum == 5) {
					coral1.generate(worldObj, rand, x, y, z);
				}
				else if (randomNum == 4) {
					coral2.generate(worldObj, rand, x, y, z);
				}
				else if (randomNum == 3) {
					coral3.generate(worldObj, rand, x, y, z);
				}
				else if (randomNum == 2) {
					coral4.generate(worldObj, rand, x, y, z);
				}
				else if (randomNum == 1) {
					coral5.generate(worldObj, rand, x, y, z);
				}
				else {
					coral6.generate(worldObj, rand, x, y, z);
				}
			}
		}
		else if (biomegenbase == DimensionOrganizer.BoreanBubble) {
			for (i = 0; i < 4; i++) {
				if (TerrainGen.populate(par1IChunkProvider, worldObj, rand, var4, var5, false, LAKE)) {
					var12 = var4 + rand.nextInt(16) + 8;
					var14 = var5 + rand.nextInt(16) + 8;
			...
			for (i = 0; i < 5; i++) {
				...
				if (randomNum == 3) {
					new BubbleTree1().generate(worldObj, rand, x, y, z);
				}
				else if (randomNum == 2) {
					new BubbleTree2().generate(worldObj, rand, x, y, z);
				}
				else if (randomNum == 1) {
					new BubbleTree3().generate(worldObj, rand, x, y, z);
				}
				else {
					new BubbleTree4().generate(worldObj, rand, x, y, z);
				}
			}
		}
		else if (biomegenbase == DimensionOrganizer.BoreanForest) {
			for (i = 0; i < 6; i++) {
				x = var4 + rand.nextInt(16);
				z = var5 + rand.nextInt(16);
				y = worldObj.getHeightValue(x, z);
			...
			for (i = 0; i < 3; i++) {
				...
				if (rand.nextInt(2) == 1) {
					new Coral7().generate(worldObj, rand, x, y, z);
				}
				else {
					new Coral8().generate(worldObj, rand, x, y, z);
				}
			}
			...
			for (i = 0; i < 65; i++) {
				...
				if (worldObj.getBlock(x, y - 1, z) == Blockizer.HardCoral && worldObj.getBlock(x, y, z) == Blocks.air) {

					if (rand.nextInt(2) == 1)
						worldObj.setBlock(x, y, z, Blockizer.OcealitesRed);
					else
						worldObj.setBlock(x, y, z, Blockizer.WaterweedsRed);
				}
			}
		}

		if (biomegenbase != DimensionOrganizer.BoreanRed)
			for (i = 0; i < 65; i++) {
				x = var4 + rand.nextInt(16);
				z = var5 + rand.nextInt(16);
				y = worldObj.getHeightValue(x, z);

				if (worldObj.getBlock(x, y - 1, z) == Blockizer.GrassBorean && worldObj.getBlock(x, y, z) == Blocks.air) {
					pick = rand.nextInt(5);

					if (pick == 1)
						worldObj.setBlock(x, y, z, Blockizer.BurealStocks);
					else if (pick == 2)
						worldObj.setBlock(x, y, z, Blockizer.WaterWeedsGreen);
					else if (pick == 3)
						worldObj.setBlock(x, y, z, Blockizer.WaterWeedsWhite);
					else if (pick == 4)
						worldObj.setBlock(x, y, z, Blockizer.WaterWeedsYellow);
					else
						worldObj.setBlock(x, y, z, Blockizer.Ocealites);
				}
			}
		...
		if (biomegenbase != DimensionOrganizer.BoreanRed)
			if (rand.nextInt(650) == 127) {
				...
				(new RuneShrinePlatform(2)).generate(worldObj, rand, x, y, z);
			}

		if (biomegenbase != DimensionOrganizer.BoreanRed)
			if (rand.nextInt(550) == 344) {
				x = var4 + rand.nextInt(16);
				z = var5 + rand.nextInt(16);
				y = worldObj.getHeightValue(x + 1, z + 1);
				if (worldObj.getBlock(x, y - 1, z) == Blockizer.GrassBorean)
					(new LottoBoreanStructure()).generate(worldObj, rand, x, y, z);
			}
		...
		if (biomegenbase != DimensionOrganizer.BoreanRed)
			if (rand.nextInt(750) == 650) {
				...
				(new FragmentTempleBorean()).generate(worldObj, rand, x, y - 1, z);
			}

		if (biomegenbase != DimensionOrganizer.BoreanRed)
			if (rand.nextInt(450) == 296) {
				x = var4 + rand.nextInt(16);
				z = var5 + rand.nextInt(16);
				y = worldObj.getHeightValue(x + 1, z + 1);
				if (worldObj.getBlock(x, y - 1, z) == Blockizer.GrassBorean)
					(new DracyonAltar()).generate(worldObj, rand, x, y, z);
			}

		if (biomegenbase != DimensionOrganizer.BoreanRed)
			if (rand.nextInt(300) == 154) {
				x = var4 + rand.nextInt(16);
				z = var5 + rand.nextInt(16);
				y = worldObj.getHeightValue(x, z);
				if (worldObj.getBlock(x, y - 1, z) == Blockizer.GrassBorean || worldObj.getBlock(x, y - 1, z) == Blockizer.DirtBorean)
					(new HydroliskStructure()).generate(worldObj, rand, x, y, z);
			}
	}
