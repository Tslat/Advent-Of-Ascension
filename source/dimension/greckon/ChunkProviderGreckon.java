...
import net.nevermine.structures.greckon.*;
...
public class ChunkProviderGreckon implements IChunkProvider {
	...
	private MapGenBase caveGenerator = new MapGenGreckonCaves();
	...
	private WorldGenerator hauntedtree1 = new HauntedTree1();
	private WorldGenerator hauntedtree2 = new HauntedTree2();
	private WorldGenerator hauntedtree3 = new HauntedTree3();
	private WorldGenerator hauntedtree4 = new HauntedTree4();

	public ChunkProviderGreckon(World par1World, long par2) {
		worldObj = par1World;
					...
					for (int l2 = 0; l2 < 8; ++l2) {
						...
						for (int i3 = 0; i3 < 4; ++i3) {
							...
							for (int k3 = 0; k3 < 4; ++k3) {
								if ((d15 += d16) > 0.0D)
									b[j3 += short1] = Blockizer.StoneGreckon;

								else if (k2 * 8 + l2 < b0)
									b[j3 += short1] = Blockizer.GrassGreckon;
								else
									b[j3 += short1] = null;
		...
		for (int l1 = 255; l1 >= 0; --l1) {
			...
			if (l1 <= 0 + p_150560_2_.nextInt(5)) {
				...
				if (block2 != null && block2.getMaterial() != Material.air) {
					if (block2 == Blockizer.StoneGreckon) {
						if (k == -1) {
							if (l <= 0) {
								block = null;
								b0 = 0;
								block1 = Blockizer.StoneGreckon;
							}
							else if (l1 >= 59 && l1 <= 64) {
								block = b.topBlock;
							...
							if (l1 >= 62) {
								...
								block = null;
								block1 = Blockizer.StoneGreckon;
								p_150560_3_[i2] = Blockizer.StoneGreckon;
							}
							else {
							...
							if (k == 0 && block1 == Blockizer.StoneGreckon) {
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		if (biomegenbase != DimensionOrganizer.GreckonForest) {
			for (i = 0; i < 1; i++) {
				...
				if (randomNum == 3) {
					hauntedtree1.generate(worldObj, rand, x, y, z);
				}
				else if (randomNum == 2) {
					hauntedtree2.generate(worldObj, rand, x, y, z);
				}
				else if (randomNum == 1) {
					hauntedtree3.generate(worldObj, rand, x, y, z);
				}
				else {
					hauntedtree4.generate(worldObj, rand, x, y, z);
				}
			}
		}
		else {
		...
		if (biomegenbase == DimensionOrganizer.GreckonSkull) {
			...
			z = var5 + rand.nextInt(16);
			y = worldObj.getHeightValue(x, z);
			if (worldObj.getBlock(x, y - 1, z) == Blockizer.GrassGreckon)
				new SkullPost().generate(worldObj, rand, x, y, z);
		}
		...
		for (i = 0; i < 15; i++) {
			...
			if ((worldObj.getBlock(x, y, z) == Blocks.air) && (worldObj.getBlock(x, y - 1, z) == Blockizer.GrassGreckon)) {
				worldObj.setBlock(x, y, z, Blockizer.HauntedFlower);
			}
		}
		...
		if (rand.nextInt(500) == 223) {
			x = var4 + rand.nextInt(16);
			z = var5 + rand.nextInt(16);
			y = worldObj.getHeightValue(x + 1, z + 1) + 25;

			if (y < 200)
				(new LottoGreckonStructure()).generate(worldObj, rand, x, y - 1, z);
		}
		...
		if (rand.nextInt(750) == 650) {
			...
			(new FragmentTempleGreckon()).generate(worldObj, rand, x, y - 1, z);
		}

		if (rand.nextInt(180) == 74) {
			x = var4 + rand.nextInt(16);
			z = var5 + rand.nextInt(16);
			y = worldObj.getHeightValue(x + 2, z + 2);
			(new FacelessTree()).generate(worldObj, rand, x, y - 1, z);
		}
		...
		if (rand.nextInt(650) == 127) {
			...
			(new RuneShrinePlatform(8)).generate(worldObj, rand, x, y, z);
		}

		if (TerrainGen.populate(par1IChunkProvider, worldObj, rand, var4, var5, false, LAKE) && rand.nextInt(4) == 0) {
			var12 = var4 + rand.nextInt(16) + 8;
		...
		for (i = 0; i < 2; i++) {
			...
			y = rand.nextInt(25) + 3;
			z = var5 + rand.nextInt(16);
			new WorldGenMinable(Blockizer.OreGhastly, 5, Blockizer.StoneGreckon).generate(worldObj, rand, x, y, z);
		}
		for (i = 0; i < 2; i++) {
			x = var4 + rand.nextInt(16);
			y = rand.nextInt(30) + 30;
			z = var5 + rand.nextInt(16);
			new WorldGenMinable(Blockizer.OreGhoulish, 5, Blockizer.StoneGreckon).generate(worldObj, rand, x, y, z);
		}
	}
