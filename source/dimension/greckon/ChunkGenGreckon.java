...
public class ChunkGenGreckon implements IChunkGenerator {
	...
	private final Biome biome = DimensionRegister.biomeGreckon;
	...
	private CaveGenGreckon caveGen = new CaveGenGreckon();
	private WorldGenLakes lakeGen = new WorldGenLakes(Blocks.WATER);

	protected ChunkGenGreckon(World world) {
		this.world = world;
			...
			for (int l = 0; l < 5; ++l) {
				...
				for (int m = -2; m <= 2; ++m) {
					for (int n = -2; n <= 2; ++n) {
						...
						baseHeight = 1 + baseHeight * 1.25f;
						heightVariation = 1 + heightVariation * 1.5f;
	...
	public void populate(int chunkX, int chunkZ) {
		...
		if (ConfigurationUtil.spawnChanceStormRuneShrine > 0 && rand.nextInt(ConfigurationUtil.spawnChanceStormRuneShrine) == 0) {
			...
			if (world.getBlockState(pos.setPos(x + 3, y, z + 3)).getBlock() == Blocks.AIR)
				StructuresHandler.generateStructure("StormRuneShrine", world, rand, pos.setPos(x, y, z));
		}
