...
public class MapGenGreckonCaves extends MapGenBase {
	...
	protected void digBlock(Block[] data, int index, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop) {
		BiomeGenBase biome = worldObj.getBiomeGenForCoords(x + chunkX * 16, z + chunkZ * 16);
		Block top = (isExceptionBiome(biome) ? Blockizer.GrassGreckon : biome.topBlock);
		Block filler = (isExceptionBiome(biome) ? Blockizer.StoneGreckon : biome.fillerBlock);
		Block block = data[index];

		if (block == Blockizer.StoneGreckon || block == filler || block == top) {
			if (y < 2) {
