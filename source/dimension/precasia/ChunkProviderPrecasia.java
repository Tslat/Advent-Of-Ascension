...
public class ChunkProviderPrecasia implements IChunkProvider {
	...
	private MapGenBase caveGenerator = new MapGenPrecasiaCaves();
	private BiomeGenBase[] biomesForGeneration;
	private double[] gen1, gen2, gen3, gen4;
	private int[][] ia = new int[32][32];

	private WorldGenerator precasiatree1 = new PrecasiaTree1();
	private WorldGenerator precasiatree2 = new PrecasiaTree2();
	private WorldGenerator precasiatree3 = new PrecasiaTree3();
	private WorldGenerator precasiatree4 = new PrecasiaTree4();

	public ChunkProviderPrecasia(World par1World, long par2) {
		worldObj = par1World;
					...
					for (int l2 = 0; l2 < 8; ++l2) {
						...
						for (int i3 = 0; i3 < 4; ++i3) {
							...
							for (int k3 = 0; k3 < 4; ++k3) {
								if ((d15 += d16) > 0.0D)
									b[j3 += short1] = Blockizer.StonePrecasianLow;

								else if (k2 * 8 + l2 < b0)
									b[j3 += short1] = Blockizer.GrassPrecasia;
								else
									b[j3 += short1] = null;
		...
		for (int l1 = 255; l1 >= 0; --l1) {
			...
			if (l1 <= 0 + p_150560_2_.nextInt(5)) {
				...
				if (block2 != null && block2.getMaterial() != Material.air) {
					if (block2 == Blockizer.StonePrecasianLow) {
						if (k == -1) {
							if (l <= 0) {
								block = null;
								b0 = 0;
								block1 = Blockizer.StonePrecasianLow;
							}
							else if (l1 >= 59 && l1 <= 64) {
								block = b.topBlock;
							...
							if (l1 < 63 && (block == null || block.getMaterial() == Material.air)) {
								if (b.getFloatTemperature(p_150560_5_, l1, p_150560_6_) < 0.15F) {
									...
									block = Blockizer.StonePrecasianLow;
									b0 = 0;
								}
							}
							...
							if (l1 >= 62) {
								...
								block = null;
								block1 = Blockizer.StonePrecasianLow;
								p_150560_3_[i2] = Blockizer.StonePrecasianLow;
							}
							else {
							...
							if (k == 0 && block1 == Blockizer.StonePrecasianLow) {
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		if (biomegenbase == DimensionOrganizer.Precasia || biomegenbase == DimensionOrganizer.PrecasiaDesert) {
			for (i = 0; i < 3; i++) {
				...
				if (randomNum == 1) {
					precasiatree1.generate(worldObj, rand, x, y, z);

				}
				else if (randomNum == 2) {
					precasiatree2.generate(worldObj, rand, x, y, z);

				}
				else if (randomNum == 3) {
					precasiatree3.generate(worldObj, rand, x, y, z);

				}
				else {
					precasiatree4.generate(worldObj, rand, x, y, z);

				}
			}

			if (biomegenbase == DimensionOrganizer.PrecasiaDesert) {
				if (rand.nextInt(3) == 1) {
					x = var4 + rand.nextInt(16);
					z = var5 + rand.nextInt(16);
					y = worldObj.getHeightValue(x + 1, z + 1);

					if (worldObj.getBlock(x + 1, y - 1, z + 1) == Blockizer.PrecasianSand) {
						int randomNum = rand.nextInt(4);

						if (randomNum == 1) {
							new AncientCactus1().generate(worldObj, rand, x, y - 1, z);
						}
						else if (randomNum == 2) {
							new AncientCactus2().generate(worldObj, rand, x, y - 1, z);
						}
						else if (randomNum == 3) {
							worldObj.setBlock(x + 1, y, z + 1, Blocks.deadbush);
						}
						else {
							new AncientCactus3().generate(worldObj, rand, x, y - 1, z);
						}
					}
		...
		else if (biomegenbase == DimensionOrganizer.PrecasiaTall) {
			x = var4 + rand.nextInt(16);
			z = var5 + rand.nextInt(16);
			y = worldObj.getHeightValue(x + 5, z + 5);

			if (worldObj.getBlock(x + 5, y - 1, z + 5) == Blockizer.GrassPrecasia) {
				int randomNum = rand.nextInt(3);
				if (randomNum == 1) {
					new EucadonTree().generate(worldObj, rand, x + 2, y - 1, z + 2);
				}
				else if (randomNum == 2) {
					new DomiguousTree1().generate(worldObj, rand, x, y - 1, z);
				}
				else {
					new DomiguousTree2().generate(worldObj, rand, x, y - 1, z);

				}
			}
		}
		else if (biomegenbase == DimensionOrganizer.PrecasiaField) {
			if (rand.nextInt(15) == 12) {
				x = var4 + rand.nextInt(16);
				z = var5 + rand.nextInt(16);
				y = worldObj.getHeightValue(x + 5, z + 5);
				if (worldObj.getBlock(x + 5, y - 1, z + 5) == Blockizer.GrassPrecasia)
					new MelumiaTree().generate(worldObj, rand, x, y - 1, z);
			}
		}

		if (biomegenbase != DimensionOrganizer.PrecasiaField) {
			for (i = 0; i < 25; i++) {
				x = var4 + rand.nextInt(16);
				z = var5 + rand.nextInt(16);
				y = worldObj.getHeightValue(x, z);

				if (worldObj.getBlock(x, y - 1, z) == Blockizer.GrassPrecasia) {

					if (rand.nextBoolean() == true)
						worldObj.setBlock(x, y, z, Blockizer.LuconGrass);
					else
						worldObj.setBlock(x, y, z, Blockizer.TangleThorns);
				}
			...
			for (i = 0; i < 25; i++) {
				...
				if (worldObj.getBlock(x, y - 1, z) == Blockizer.GrassPrecasia) {
					...
					if (pick < 3)
						worldObj.setBlock(x, y, z, Blockizer.Pertonias);
					else if (pick < 6)
						worldObj.setBlock(x, y, z, Blockizer.Mellians);
					else if (pick == 6)
						worldObj.setBlock(x, y, z, Blockizer.LuconGrass);
					else
						worldObj.setBlock(x, y, z, Blockizer.TangleThorns);
		...
		if (biomegenbase != DimensionOrganizer.PrecasiaDesert)
			if (rand.nextInt(650) == 127) {
				...
				(new RuneShrinePlatform(3)).generate(worldObj, rand, x, y, z);
			}

		if (biomegenbase == DimensionOrganizer.Precasia)
			if (rand.nextInt(850) == 335) {
				x = var4 + rand.nextInt(16);
				z = var5 + rand.nextInt(16);
				y = worldObj.getHeightValue(x + 1, z + 1);
				(new OpteryxNest()).generate(worldObj, rand, x, y, z);
			}

		if (biomegenbase == DimensionOrganizer.Precasia)
			if (rand.nextInt(420) == 223) {
				x = var4 + rand.nextInt(16);
				z = var5 + rand.nextInt(16);
				y = worldObj.getHeightValue(x + 1, z + 1);
				if (worldObj.getBlock(x, y - 1, z) == Blockizer.GrassPrecasia)
					(new LottoPrecasiaStructure()).generate(worldObj, rand, x, y - 1, z);
			}
