...
public class ChunkGenIromine implements IChunkGenerator {
	...
	private final Biome biome = DimensionRegister.biomeIromine;
	...
	private CaveGenIromine caveGen = new CaveGenIromine();

	protected ChunkGenIromine(World world) {
		this.world = world;
						...
						for (int k2 = 0; k2 < 4; ++k2) {
							...
							for (int l2 = 0; l2 < 4; ++l2) {
								if ((lvt_45_1_ += d16) > 0.0D) {
									primer.setBlockState(i * 4 + k2, i2 * 8 + j2, l * 4 + l2, BlockRegister.stoneIromine.getDefaultState());
								}
	...
	private void generateBiomeTerrain(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x, int z, double noiseVal) {
		...
		for (int j1 = 255; j1 >= 0; --j1) {
			if (j1 <= 2) {
				...
				else if (iblockstate2.getBlock() == BlockRegister.stoneIromine) {
					if (j == -1) {
						if (k <= 0) {
						...
						if (j1 < seaLevel && (topBlock == null || topBlock.getMaterial() == Material.AIR)) {
							if (biome.getTemperature(tempCheckPos.setPos(x, j1, z)) < 0.15F) {
								...
								topBlock = BlockRegister.stoneIromine.getDefaultState();
							}
						}
	...
	public void populate(int chunkX, int chunkZ) {
			...
			switch (rand.nextInt(4)) {
				case 0:
					...
					if (world.getBlockState(pos.setPos(x + 6, y - 1, z + 6)) == biome.topBlock)
						StructuresHandler.generateStructure("EnforcerTower", world, rand, pos.setPos(x, y, z));
					break;
				case 1:
					x = baseX + rand.nextInt(16);
					z = baseZ + rand.nextInt(16);
					y = world.getHeight(x + 5, z + 6);

					if (world.getBlockState(pos.setPos(x + 5, y - 1, z + 6)) == biome.topBlock)
						StructuresHandler.generateStructure("MechyonTemple", world, rand, pos.setPos(x, y, z));
					break;
				case 2:
					x = baseX + rand.nextInt(16);
					z = baseZ + rand.nextInt(10);
					y = world.getHeight(x + 7, z + 10);

					if (world.getBlockState(pos.setPos(x + 7, y - 1, z + 10)) == biome.topBlock)
						StructuresHandler.generateStructure("IroPassage1", world, rand, pos.setPos(x, y, z));
					break;
				case 3:
					x = baseX + rand.nextInt(13);
					...
					if (world.getBlockState(pos.setPos(x + 8, y - 1, z + 8)) == biome.topBlock)
						StructuresHandler.generateStructure("IroPassage2", world, rand, pos.setPos(x, y, z));
					break;
			}
		}
		...
		if (ConfigurationUtil.StructureConfig.iromine.energyRuneShrineSpawnChance > 0 && rand.nextInt(ConfigurationUtil.StructureConfig.iromine.energyRuneShrineSpawnChance) == 0) {
			...
			if (world.getBlockState(pos.setPos(x + 3, y, z + 3)).getBlock() == Blocks.AIR)
				StructuresHandler.generateStructure("EnergyRuneShrine", world, rand, pos.setPos(x, y, z));
		}
