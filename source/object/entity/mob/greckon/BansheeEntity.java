	...
	protected void onAttack(Entity target) {
		if (target instanceof LivingEntity) {
			if (!(target instanceof PlayerEntity) || !((PlayerEntity)target).inventory.contains(new ItemStack(AoAItems.SCREAM_SHIELD.get()))) {
				EntityUtil.applyPotions(target, new PotionUtil.EffectBuilder(Effects.MOVEMENT_SLOWDOWN, 150).level(6),
						new PotionUtil.EffectBuilder(Effects.WEAKNESS, 150).level(6),
						new PotionUtil.EffectBuilder(Effects.DIG_SLOWDOWN, 150).level(6));
			}
		}
	}
}
