...
public class EntityGorbCitizen extends EntityVillager implements INpc, IMerchant {
	...
	public EntityGorbCitizen(final World var1) {
		super(var1);
		randomTickDivider = 0;
		isMating = false;
		isPlaying = false;
	...
	public boolean interact(final EntityPlayer var1) {
		if (isEntityAlive() && !isTrading() && !isChild()) {
			if (!worldObj.isRemote) {
				...
				var1.addChatMessage(StringUtil.getLocale("message.dialogue.gorbCitizen." + rand.nextInt(5)));
			}
			return true;
		}
