...
public class PlayerUtil {
    ...
    public static void messageAllPlayersInRange(ITextComponent msg, World world, BlockPos center, int radius) {
        for (PlayerEntity pl : WorldUtil.getAllPlayersInRegion(world, new AxisAlignedBB(center).grow(radius))) {
            pl.sendMessage(msg);
        }
    }
    ...
    public static BlockPos getBlockAimingAt(PlayerEntity pl, double distance) {
        Vec3d startVec = new Vec3d(pl.getPosX(), pl.getPosY() + (double)pl.getEyeHeight(), pl.getPosZ());
        ...
        BlockRayTraceResult ray = pl.world.rayTraceBlocks(new RayTraceContext(startVec, endVec, RayTraceContext.BlockMode.COLLIDER, RayTraceContext.FluidMode.ANY, null));
    ...
    public static void playSoundForPlayer(ServerPlayerEntity player, SoundEvent sound, SoundCategory category, double posX, double posY, double posZ, float volume, float pitch) {
        player.connection.sendPacket(new SPlaySoundEffectPacket(sound, category, posX, posY, posZ, volume, pitch));
    }
