...
public class ShadowlordEntity extends MonsterEntity implements IRangedAttackMob, AoARangedAttacker {
	...
	private static final DataParameter<Integer> FIRST_HEAD_TARGET = EntityDataManager.<Integer>createKey(ShadowlordEntity.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> SECOND_HEAD_TARGET = EntityDataManager.<Integer>createKey(ShadowlordEntity.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> THIRD_HEAD_TARGET = EntityDataManager.<Integer>createKey(ShadowlordEntity.class, DataSerializers.VARINT);
	...
	protected void registerAttributes() {
		...
		this.getAttribute(SharedMonsterAttributes.FOLLOW_RANGE).setBaseValue(52);
	}
	...
	public boolean isInvulnerableTo(DamageSource source) {
		return source.getTrueSource() instanceof ShadowlordEntity || DamageUtil.isMagicDamage(source, this, 1) || source.isExplosion() || source.isFireDamage() || super.isInvulnerableTo(source);
	}
	...
	public void onDeath(DamageSource cause) {
		...
		if (!world.isRemote) {
			...
			if (killer != null)
				PlayerUtil.messageAllPlayersInRange(LocaleUtil.getLocaleMessage("message.mob.shadowlord.kill", killer.getDisplayName().getFormattedText()), world, getPosition(), 50);
		}
	}
	...
	public void doProjectileImpactEffect(BaseMobProjectile projectile, Entity target) {
		if (target instanceof LivingEntity) {
			...
			EntityUtil.applyPotions(target, new PotionUtil.EffectBuilder(Effects.NAUSEA, 100));
		}
