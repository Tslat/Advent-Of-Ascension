...
public class EntityNethengeicWither extends EntityMob implements BossEntity, IRangedAttackMob, SpecialPropertyEntity, AoARangedAttacker {
	...
	public EntityNethengeicWither(World world) {
		...
		this.setSize(entityWidth, 3.28125f);
	...
	protected void initEntityAI() {
		...
		this.targetTasks.addTask(2, new EntityAINearestAttackableTarget(this, EntityLivingBase.class, 0, false, false, PredicateUtil.NOT_UNDEAD));
		this.targetTasks.addTask(3, new EntityAIFindEntityNearestPlayer(this));
	}
	...
	private void shootAtBlockPos(int head, double posX, double posY, double posZ) {
		if (attackCooldown > 0) {
			...
			BaseMobProjectile projectile = new EntityNethengeicWitherShot(this, getStage() == 3);
	...
	public boolean isEntityInvulnerable(DamageSource source) {
		return source.getTrueSource() instanceof EntityNethengeicWither || EntityUtil.isMagicDamage(source, this, 1) || source.isExplosion() || source.isFireDamage() || super.isEntityInvulnerable(source);
	}
	...
	public void onDeath(DamageSource cause) {
		...
		if (!world.isRemote) {
			...
			if (killer != null)
				StringUtil.sendMessageWithinRadius(StringUtil.getLocaleWithArguments("message.mob.nethengeicWither.kill", killer.getDisplayNameString()), this, 50);
		}
	}

	@Override
	public void doProjectileEntityImpact(BaseMobProjectile projectile, Entity target) {
		if (target.attackEntityFrom(DamageSource.causeIndirectDamage(projectile, this), projectile instanceof EntityNethengeicWitherShot ? ((EntityNethengeicWitherShot)projectile).cataclysmic ? 30 : 20 : 20))
	...
	public void doProjectileImpactEffect(BaseMobProjectile projectile, Entity target) {
		if (target instanceof EntityLivingBase) {
			if (projectile instanceof EntityNethengeicWitherShot) {
				if (((EntityNethengeicWitherShot)projectile).cataclysmic) {
					((EntityLivingBase)target).addPotionEffect(new PotionEffect(MobEffects.WITHER, 70, 2, true, true));
				}
				else {
					((EntityLivingBase)target).addPotionEffect(new PotionEffect(MobEffects.WITHER, 90, 1, true, true));
				}
			}
		}
