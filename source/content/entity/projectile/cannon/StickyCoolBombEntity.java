...
public class StickyCoolBombEntity extends PhysicalWeaponProjectile {
	protected BlockPos stuckTo;
	protected int countdown = 80;

	public StickyCoolBombEntity(EntityType<? extends StickyCoolBombEntity> entityType, Level level) {
		super(entityType, level);
	}

	public StickyCoolBombEntity(EntityType<? extends StickyCoolBombEntity> entityType, Level level, WeaponFiringContext context) {
		super(entityType, level, context);
	}

	public StickyCoolBombEntity(Level level, WeaponFiringContext context) {
		this(AoAProjectiles.STICKY_COOL_BOMB.get(), level, context);
	}
	...
	protected void explode(Vec3 position) {
		if (level() instanceof ServerLevel level)
			AoAExplosionBuilder.at(level, position, AoAExplosions.blastCannon(target -> getShotContext().weaponStack().getItem() instanceof AoACannon cannon ? cannon.addCannonDamageBonus(1, target) : 1f),
