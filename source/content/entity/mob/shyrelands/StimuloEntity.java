    ...
    public void aiStep() {
        ...
        if (!level.isClientSide) {
            ...
            if (healthPercent < 0.25) {
                effect = new EffectBuilder(Effects.MOVEMENT_SPEED, -1).level(1);
            }
            else if (healthPercent < 0.50) {
                effect = new EffectBuilder(Effects.MOVEMENT_SPEED, -1).level(2);
            }
            else if (healthPercent < 0.75) {
                effect = new EffectBuilder(Effects.MOVEMENT_SPEED, -1).level(3);
            }
