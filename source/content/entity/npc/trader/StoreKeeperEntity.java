...
public class StoreKeeperEntity extends AoATrader {
	private static final Int2ObjectMap<VillagerTrades.ITrade[]> TRADES = new TradeListBuilder()
			...
			.trades(2,
					BuildableTrade.trade(AoABlocks.TOXIC_LOG, 6).cost(AoAItems.COPPER_COIN, 20).xp(12))
			.trades(3,
	...
	public static ItemStack poisonPotionStack() {
		return new PotionBuilder(Items.LINGERING_POTION).addEffect(new EffectBuilder(Effects.POISON, 600).build()).withTranslationKey("item.minecraft.lingering_potion.effect.poison").build();
	}
}
