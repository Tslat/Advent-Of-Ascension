...
public class PlutonStickler extends AoACannon {
	...
	public WeaponProjectile createProjectileEntity(Level level, WeaponFiringContext context) {
		return new PlutonSticklerShotEntity(level, context);
	}
	...
	protected void onDamageEntity(Level level, WeaponProjectile projectile, RayTrace<?> rayTrace, Entity hitEntity, float damage) {
		if (hitEntity instanceof LivingEntity target && target.isAlive())
			level.addFreshEntity(new PlutonSticklerStuckEntity(level, projectile.getShooter(), target, target.position().vectorTo(rayTrace.hitPos()), projectile.getShotContext()));
	}

	@Override
	public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
		super.appendHoverText(stack, context, tooltip, flag);
		tooltip.add(2, LocaleUtil.getFormattedItemDescriptionText(this, LocaleUtil.ItemDescriptionType.UNIQUE, 1));
