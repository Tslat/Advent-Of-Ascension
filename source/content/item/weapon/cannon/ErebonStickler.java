...
public class ErebonStickler extends AoACannon {
	public ErebonStickler(Item.Properties properties) {
		super(properties);
	}

	@Override
	public WeaponProjectile createProjectileEntity(Level level, WeaponFiringContext context) {
		return new ErebonSticklerShotEntity(level, context);
	}
	...
	protected void onDamageEntity(Level level, WeaponProjectile projectile, RayTrace<?> rayTrace, Entity hitEntity, float damage) {
		if (hitEntity instanceof LivingEntity target && target.isAlive())
			level.addFreshEntity(new ErebonSticklerStuckEntity(level, projectile.getShooter(), target, target.position().vectorTo(rayTrace.hitPos()), projectile.getShotContext()));
	}
