...
public class LuxonStickler extends AoACannon {
	public LuxonStickler(Item.Properties properties) {
		super(properties);
	}

	@Override
	public WeaponProjectile createProjectileEntity(Level level, WeaponFiringContext context) {
		return new LuxonSticklerShotEntity(level, context);
	}
	...
	protected void onDamageEntity(Level level, WeaponProjectile projectile, RayTrace<?> rayTrace, Entity hitEntity, float damage) {
		if (hitEntity instanceof LivingEntity target && target.isAlive())
			level.addFreshEntity(new LuxonSticklerStuckEntity(level, projectile.getShooter(), target, target.position().vectorTo(rayTrace.hitPos()), projectile.getShotContext()));
	}
