...
public class Flowercorne extends AoABlaster<WeaponProjectile> {
	...
	void fireBlaster(ServerLevel level, WeaponFiringContext context) {
		fireBlasterProjectile(level, context, FlowerShotEntity::new, shot -> shot.fromArmPosWithOffset(0, 0.25f).shootingAtTarget(context.velocity(), context.inaccuracy()));
		fireBlasterProjectile(level, context, FlowerShotEntity::new);
	}
	...
	protected void onDamageEntity(Level level, WeaponProjectile effect, WeaponFiringContext context, RayTrace<?> rayTrace, Entity hitEntity, float damage) {
		EntityUtil.applyPotions(hitEntity, context.getShooter(), new EffectBuilder(MobEffects.WEAKNESS, 100));
	}
