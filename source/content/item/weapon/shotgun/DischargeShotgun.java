...
public class DischargeShotgun extends AoAShotgun {
	...
	protected void explode(Level level, Vec3 position, @Nullable Entity shooter, Entity pellet) {
		if (level instanceof ServerLevel serverLevel)
			AoAExplosionBuilder.at(serverLevel, position, AoAExplosions.DISCHARGE_SHOTGUN, StandardExplosion::new).explodingEntity(pellet).explode();
	}

	@Override
	public WeaponProjectile createProjectileEntity(Level level, WeaponFiringContext context) {
		return new PhysicalWeaponProjectile(AoAProjectiles.DISCHARGE_PELLET.get(), level, context);
	}
	...
	public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
		...
		for (MutableComponent component : LocaleUtil.getExplosionInfoLocale(AoAExplosions.DISCHARGE_SHOTGUN, true, flag.isAdvanced(), false)) {
