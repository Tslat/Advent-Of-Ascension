package net.tslat.aoa3.content.world.genold2.feature;
...
public class VanillaJsonFeature extends Feature<VanillaJsonFeature.VanillaJsonFeatureConfig> {
	...
	public boolean place(FeaturePlaceContext<VanillaJsonFeatureConfig> context) {
		...
		return level.registryAccess().registryOrThrow(Registries.PLACED_FEATURE).get(context.config().feature()).place(level, context.chunkGenerator(), context.random(), context.origin());
	}

	public record VanillaJsonFeatureConfig(ResourceKey<PlacedFeature> feature) implements FeatureConfiguration {
		public static final Codec<VanillaJsonFeatureConfig> CODEC = ResourceKey.codec(Registries.PLACED_FEATURE)
