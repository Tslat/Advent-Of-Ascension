...
public class LunarCreationTableBlockEntity extends BlockEntity implements Nameable, MenuProvider {
	...
	public void load(CompoundTag compound) {
		...
		if (compound.contains("contents", Tag.TAG_COMPOUND)) {
			legacyDeserializeContents(compound.getCompound("contents"));
		}
		else {
			this.items.clear();
	...
	private void legacyDeserializeContents(CompoundTag contentsTag) {
		this.items.clear();

		for (int i = 0; i < 9; i++) {
			if (contentsTag.contains(String.valueOf(i)))
				this.items.set(i, ItemStack.of(contentsTag.getCompound(String.valueOf(i))));
		}
	}
}
