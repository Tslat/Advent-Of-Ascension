...
public class AshfernCropBlock extends AoACropBlock {
	...
	public boolean canSurvive(BlockState state, LevelReader level, BlockPos pos) {
		if (level instanceof WorldGenRegion) { // Because of a vanilla bug with lighting levels in chunkgen
			if (!level.getBlockState(pos).isAir())
				return false;
		}
		else {
			if (level.getRawBrightness(pos, 0) < 8 && !level.canSeeSky(pos))
				return false;
		}

		return level.getBlockState(pos.below()).is(Blocks.BASALT);
	}
}
